...
**Boosters** (`open boosters`, once revealed) → **Compute** (`open compute`, once revealed) →
**Data** (`open data`, page id `'game'`, only once `mainGameUnlocked`) → **Guide** (`open guide`,
always) → **More** (`open more menu`, always —
...
must not depend on unlocking Data: **Milestones** and **Settings**. Destructive **Reset** / **Reset
...
full and `isDiskCacheBlockReleasable` — manual transfer to Data Bits only; never auto), then a
