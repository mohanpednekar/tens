...
`App.jsx`'s shared `components/AppNav` (Data / Foundry / Storage / Compute / Guide) — pages
...
omits Data during the gate so there is still no escape hatch via the production screen
...
- A `CacheBlocksRow` (`role="group"`, `aria-label="<size> disk array cache"`) of exactly
  ...
  `"transfer <size> cache block N to Data Bits"` when releasable, else the plain
  `"<size> cache block N"`; `title` names the manual-only Data Bits transfer once releasable;
...
- **Page title.** The MainPage header is `<h1>Data</h1>` (screen name; AppNav short label **Data** — not the game name
  “Tens”). Top-level destinations (Data / Foundry / Storage / Compute / Guide) live in
...
tracks (`aria-valuenow = min(era.count, milestone)`). The two tier categories list all ten Data tiers via `getAutobuyerUnlockMilestone`/
