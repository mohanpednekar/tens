...
4. Whenever Memory is **full** (`bits === capacity`) **and** no other currently-possible action is
   ...
   `INTRO_CAPACITY_CAP_BITS` (512 KiB / 4,194,304 bits, the largest power of two strictly below
   1 MiB for this generator) — once at the cap, Sacrifice is permanently unavailable
...
8. **Disks** (`disks: { [capacityBits]: currentlyFullCount }`, `disksBuiltTotal:
   ...
   Byte-accurate bits — a real "1 KB" disk is 8000 bits; then 10 KB, 100 KB, **1 MB**, 10 MB, … —
   see `getDiskLadderSizeBits` / issue #368), advancing to the next size once `DISK_ARRAY_LADDER_CAP`
...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeCoresEverEarned`/`intro.computeNodes`,
   ...
   `purchaseBoosterFromDataLake`, which spends deposited Disk stock from a matching Data Lake
   instead and is unrelated to Memory/capacity entirely. Depositing into a Data Lake
   ...
   effective capacity rather than needing a separate field for it: **9** (`DATA_LAKE_SLOT_MAX`) once
   ...
   — see `docs/DESIGN_HISTORY.md`. `ComputePage` (page
   id `'boosters'`) reveals once `capacity` reaches `INTRO_COMPUTE_CORE_UNLOCK_CAPACITY` (2,097,152
   bits, "256 KiB" in Memory's own binary display scale — half of `INTRO_CAPACITY_CAP_BITS`, one
   Sacrifice doubling short of pool 1's hard cap — `isComputeCoreConversionUnlocked`). Every
   successful tier-1 Booster purchase also increments `intro.computeCoresEverEarned`, a lifetime
   ...
   **Merging Cores upward** (`intro.computeNodes`/`computeClusters`/`computeNetworks`/`computeGrids`/
   ...
   Cores themselves (via `purchaseBoosterFromDataLake`, described above) is entirely separate from
   merging them.
   ...
   **`ComputePage` shows two rows per tier** (issue #321): row 1 is the tier's name/symbol plus
   ...
   latch flips true (inside `latchComputeMergePageIfNeeded`, called from `purchaseBoosterFromDataLake`
   — the only place it ever changes) the instant `intro.computeCoresEverEarned` — the lifetime
...
```js
{
  ...
  intro: {                                                // the Byte Foundry screen's own state — see "Byte
    ...
    capacity: 8,                                          // PERMANENT. INTRO_STARTING_CAPACITY default (1
                                                          ...
                                                          // INTRO_CAPACITY_CAP_BITS (512 KiB). Displayed in
    ...
    computeCores: 0,                                      // PERMANENT, normally capped at COMPUTE_ENTITY_CAP (10)
                                                          // but purchaseBoosterFromDataLake can push it past that
                                                          // (Data-Lake-limited, not inventory-capped). Granted by
                                                          // purchaseBoosterFromDataLake (tier 1) — spending
                                                          // deposited Disk stock from that tier's Data Lake. Spent
                                                          // 1 at a time by activateComputeBoost below
    computeCoresEverEarned: 0,                            // PERMANENT, UNCAPPED lifetime counter — incremented
                                                          ...
                                                          // purchaseBoosterFromDataLake call, but never decremented
                                                          // by spending/merging. computeMergePageUnlocked below
    ...
    computeMergePageUnlocked: false,                      // PERMANENT, one-time reveal latch for ComputePage —
                                                          ...
                                                          // purchaseBoosterFromDataLake) the instant
...
| `isComputeCoreConversionUnlocked` | `state → bool` | Byte Foundry Compute Cores predicate (not a reducer): `intro.capacity >= INTRO_COMPUTE_CORE_UNLOCK_CAPACITY` (2,097,152 — 256 KiB in Memory's own binary scale) — drives whether `ByteFoundryPage` shows the "⚡ Compute" nav button to `ComputePage` at all. Unrelated to Disks entirely (earlier versions gated on Disk array fullness, then on a dynamic Memory flush — see `docs/DESIGN_HISTORY.md`) |
...
- `INTRO_CAPACITY_CAP_BITS = 4,194,304` (512 KiB) — pool 1's generator capacity ceiling, the largest power of two strictly below 1 MiB; `isMemoryCapacityUpgradeAvailable` returns `false` from here on
...
- `INTRO_COMPUTE_CORE_UNLOCK_CAPACITY = INTRO_CAPACITY_CAP_BITS / INTRO_CAPACITY_DOUBLING_STEP = 2_097_152` — Byte Foundry Compute Cores: capacity threshold (256 KiB in Memory's own binary scale, half of pool 1's hard cap — one Sacrifice doubling short of it) at which `ByteFoundryPage`'s "Compute" section/`ComputePage` becomes visible — retuned from a flat `8_000_000` under the old ×10-forever capacity ladder, since that value now sits above the new cap and would otherwise be unreachable; unrelated to Disks (see `isComputeCoreConversionUnlocked`)
