...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeNodes`, both PERMANENT, carried over
   ...
   `isComputeCoreConversionUnlocked(state)`; Compute Boost below is the first mechanic that
   actually spends Compute Cores.
   ...
   **Merging Cores upward** (`intro.computeClusters`/`computeNetworks`/`computeGrids`, all
   ...
   counters themselves), gates a separate page, `ComputePage` (`src/pages/ComputePage/index.jsx`,
   reachable via MainPage's own "🖥️ Compute" header link once unlocked, alongside "⚙️ Byte
   Foundry"/"ℹ️ Guide" — see "Architecture" in CLAUDE.md), where the three merge buttons actually
   live; `ByteFoundryPage`'s own "Compute" section deliberately keeps showing only Cores/Nodes, not
   Clusters/Networks/Grids, so the two pages don't duplicate the same counters. The latch flips
   true the instant `tickComputeCoreConversion` brings the CUMULATIVE total of Compute Cores ever
   earned to `COMPUTE_CORES_PER_NODE` (8) for the first time — checked on the post-increment total
   inside `tickComputeCoreConversion` itself, before `tickComputeNodeConversion` (called
   immediately after, same tick) converts that 8th Core back down into the very first Node — which
   is exactly why this tracks the cumulative total rather than the live `computeCores` balance:
   merging Nodes back down (or spending a Core on a Boost) must never re-hide the page once it's
   been revealed.
