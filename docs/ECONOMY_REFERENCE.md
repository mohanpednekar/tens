...
records total activations; the per-tier map drives production, while the latter drives only the
...
fixed-step ladder beyond a small floor: `getOverclockRequirement(overclockCount)` requires level 5 for the first claim, then at least three levels beyond the previous claim (`OVERCLOCK_REQUIREMENT_STEP = 3`) — a three-level-per-use cadence, unconditional (unlike
...
```js
{
  ...
  scaleUpCount: 0,                                       // RUN-SCOPED count of Scale Up activations (see
                                                          // scaleUpGame below) — total activation count for progression/status;
                                                          // per-tier production bonuses live in scaleUpTierCounts.
                                                          // See scaleUpTargetTierIndex below for
...
| `getScaleUpMultiplier` | `count → number` | `SCALE_UP_MULTIPLIER_BASE ** count`; used by `getTierScaleUpMultiplier` with a tier’s own activation count |
| `getTierScaleUpMultiplier` | `(state, tierId) → number` | Reads `scaleUpTierCounts[tierId]`, with the legacy global count as an old-save fallback, so Scale Up affects only claims that included that tier |
...
| `getScaleUpRequirement` | `state → number` | A flat level 3 for the current target tier. Before the final tier this both qualifies Scale Up and reveals the next tier; at the final tier every reset begins another fresh three-level climb. |
