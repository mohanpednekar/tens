...
8. **Storage** (`storageBanks: { [capacityBits]: currentlyFullCount }`, `storageBanksBuiltTotal:
   ...
   **not** touch `storageBanks` (the currently-full count) at all. Ranked third in the forced
   ...
   auto-filled again.
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeNodes`, both PERMANENT, carried over
   ...
   **Merging Cores upward** (`intro.computeClusters`/`computeNetworks`/`computeGrids`, all
   ...
   latch flips true the instant `tickComputeCoreConversion` brings the CUMULATIVE total of Compute
   Cores ever earned to `COMPUTE_CORES_PER_NODE` (8) for the first time — checked on the
   post-increment total inside `tickComputeCoreConversion` itself, before `tickComputeNodeConversion`
   (called immediately after, same tick) converts that 8th Core back down into the very first Node —
   which is exactly why this tracks the cumulative total rather than the live `computeCores`
   balance: merging Nodes back down (or spending a Core on a Boost) must never re-hide the merge
   section once it's been revealed.
...
```js
{
  ...
  intro: {                                                // the Byte Foundry screen's own state — see "Byte
    ...
    computeMergePageUnlocked: false,                      // PERMANENT, one-time reveal latch for ComputePage —
                                                          ...
                                                          // tickComputeCoreConversion the instant the CUMULATIVE
                                                          // Core total (not the live balance) first reaches
...
| `tickComputeNodeConversion` | `state → state` | Byte Foundry Compute Cores: same-reference no-op below `COMPUTE_CORES_PER_NODE` (8) Cores, or once `intro.computeNodes` is already at `COMPUTE_ENTITY_CAP` (10); otherwise converts complete groups of 8 into Nodes, capped at however many Nodes still fit under `COMPUTE_ENTITY_CAP` (`roomForNodes`) — a Core surplus beyond what fits is left unconverted in `intro.computeCores` rather than overflowing `computeNodes` past the cap. Called from `tickGame` right after `tickComputeCoreConversion`, so freshly-minted Cores convert the same tick they're earned. Also the only place `intro.computeMergePageUnlocked` — evaluated one call earlier, inside `tickComputeCoreConversion` itself, on the post-increment `computeCores` total, before this function spends it back down — ever flips true |
