...
genuine call. It also clears any `diskBuildQueued` its own call arms, since provisionDisk's
auto-continue would otherwise hand `tickQueuedDiskBuild` — which has no knowledge of
`foundryResetCaps` — a green light to keep funding past the cap on later ticks with no manual click.
`normalizePoolMemoryCapacity`'s own load-time `diskBuildQueued` auto-arm (see below) checks the same
`getDiskReplayPassAllowance` boundary, so reloading a save mid-replay can't reach the same gap via
the save-load path instead.
...
| `tickQueuedDiskBuild` | `state → state` | If `diskBuildQueued` is set, calls `provisionDisk` (deferring to its own `isProvisionDiskTurnAvailable` guard rather than duplicating it) — fires and clears the queue once affordable, otherwise a same-reference no-op leaving it armed. Called from `tickGame`'s `tickStorage`, ranked the same as an ordinary manual click. Always resolves `getDiskReplayPassAllowance(state, getDiskSize(state))` and passes it as `provisionDisk`'s `maxPasses` (`Infinity` outside an active `foundryResetCaps` replay, so ordinary play is unaffected); at the exact allowance boundary (`0`) clears `diskBuildQueued` directly instead of calling `provisionDisk` with a zero `maxPasses` — the ONE chokepoint every automatic (no-click) continuation path funnels through, regardless of what armed the queue (see `docs/DESIGN_HISTORY.md`'s "round 4" entry on PR #608) |
