...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeCoresEverEarned`/`intro.computeNodes`,
   ...
   `INTRO_DISK_UNLOCK_CAPACITY` reveal — `isComputeCoreConversionUnlocked`), Memory automatically
   ...
   Runs every tick (`tickGame`) right after `tickDiskAutoFill` and before `tickIntroAutoInvest`,
   so it claims Memory ahead of ordinary Kilobyte conversion once unlocked and full — the same
   "first claim" priority `tickDiskAutoFill` itself already has over `tickIntroAutoInvest`.
...
10. **Compute Boost** (`intro.computeBoostType`/`computeBoostStacks`/`computeBoostRemainingSeconds`,
   ...
   there too. Ranked fourth in the forced priority order (below Disk Fill/Bandwidth/Disk
...
`tickGame` unconditionally drives `tickIntroProduction`/`tickDiskBuild`/`tickIntroAutoInvest`/`tickDiskAutoFill`/
`tickDiskAutoRedeem` (the Byte Foundry) alongside the main-game tiers on every call, this same 50%
