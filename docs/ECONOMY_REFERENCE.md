...
step 8 below) and **Compute Cores/Nodes** (`computeCores`/`computeNodes`, see step 9 below) are
...
7. All conversions — block clicks (`convertIntroBitsToKilobytes`) and the auto-convenience below —
   ...
   **There is no per-cycle cap on how many conversions can happen.** (An earlier design
   ...
   tick instead of looping unboundedly in one call. `tickStorageAutoFill` (step 8) runs first, ahead
   of `tickIntroAutoInvest`, within `tickGame` — see the `tickGame` table row below — so a Storage
   bank the player has already built and is waiting to fill gets first claim on fresh Memory, rather
...
8. **Storage** (`storageBanks: { [capacityBits]: currentlyFullCount }`, `storageBanksBuiltTotal:
   ...
   *ever* been built at the current one, read from `storageBanksBuiltTotal` (a cumulative counter
   ...
   `docs/DESIGN_HISTORY.md`). Because `getCostEpochExponent`'s Fibonacci exponent sequence
   (1, 2, 3, 5, 8, …) skips values as `tier01` levels grow, this ladder skips sizes too: `tier01`
   level 4 costs 10,000,000 bits ("10 MB"), never 1,000,000 ("1 MB"), so a 1 MB bank can never exist.
   ...
   **not** touch `storageBanks` (the currently-full count) at all. Ranked third in the forced
   ...
   A currently-**full** bank is redeemable only once its size **exactly matches** tier01's *current*
   per-unit level cost (`isStorageBankRedeemable`, using `===` not `<=`) — a genuine one-tick-only exact
   match, not "at or below." (An earlier version used `<=`, treating any bank at or under the current
   price as redeemable; that let a player over-redeem a small bank's contents at a price higher than
   the bank was actually sized for, the same undervaluation problem the transfer block's own move to a
   dynamic cost above fixes — see `docs/DESIGN_HISTORY.md`.) Because tier01's own autobuyer can
   complete more than one level in a single tick (a banked attempt budget catching up after a
   broke/paused stretch), a burst can jump the level straight past the one a bank was sized for without
   its price ever exactly equaling that size mid-tick — such a bank simply waits, still full and not
   lost: `getFirstTierCost` only ever grows with level *within* a cycle, so the next Prestige/Speed
   Up/Overclock resets tier01's level back down, and its price grows back up through that exact value
   again, making the bank redeemable once more — the "never lost" guarantee below still holds, just
   possibly on a later cycle rather than immediately. `redeemStorageBank(capacityBits)` then
...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeCoresEverEarned`/`intro.computeNodes`,
   all PERMANENT, carried over every real Prestige exactly like the Byte generator/Storage above) — an earlier version of this
   ...
   `INTRO_STORAGE_UNLOCK_CAPACITY` reveal — `isComputeCoreConversionUnlocked`), a full Memory
   ...
   independent of tick timing entirely) runs every tick (`tickGame`) right after `tickStorageAutoFill`
   ...
   `tickStorageAutoFill` itself already has over `tickIntroAutoInvest`. `tickComputeNodeConversion`
   itself is unconditional every tick regardless of `autoClaimCoreEnabled` (it just converts whatever
   `computeCores` already holds), so a Core minted via automatic conversion converts into a Node the
   same tick, while one minted via a manual `claimComputeCore` click converts on the very next
   regular tick instead — every complete group of `COMPUTE_CORES_PER_NODE` (8) Compute Cores
   converts into 1 Compute Node, banking any remainder below that. Both bypass `isProductionFrozen`,
   same posture as every other Byte Foundry mechanic (a separate currency pool, not
   `resources.base`). Every successful conversion (manual or automatic) also increments
   `intro.computeCoresEverEarned`, a lifetime counter tracked alongside `computeCores` but never
   decremented by spending (`activateComputeBoost`) or merging (`tickComputeNodeConversion` and the
   manual merges below) — see the reveal-latch paragraph further down for why this exists as a
   ...
   **Merging Cores upward** (`intro.computeClusters`/`computeNetworks`/`computeGrids`/
   `computeFabrics`/`computeClouds`/`computeDatacenters`/`computeSupercomputers`/
   `computeMegacomputers`, all PERMANENT like `computeCores`/`computeNodes` above; see issue #280)
   extends the same ladder eight tiers further: 8 Nodes → 1 Cluster, 8 Clusters → 1 Network, 8
   Networks → 1 Grid, 8 Grids → 1 Fabric, 8 Fabrics → 1 Cloud, 8 Clouds → 1 Datacenter, 8
   Datacenters → 1 Supercomputer, 8 Supercomputers → 1 Megacomputer (`COMPUTE_MERGE_RATIO = 8`, the
   same ratio as `COMPUTE_CORES_PER_NODE` above, but a separate constant since this chain is
   conceptually distinct). "Compute" is the page/feature name only — every individual entity's
   display name drops it (`Core`/`Node`/`Cluster`/…, never "Compute Core"/"Compute Node"/…); the
   `compute`-prefixed field/function names above are internal identifiers, unrelated to what the
   player sees. Unlike Core→Node above, **none of these eight merges ever fire automatically on
   tick** — each is a manual, player-clicked action (`mergeComputeNodesIntoCluster`/
   ...
   `engine.js`, all built off the same `mergeComputeEntities(inputField, outputField)` factory), so
   the player decides whether to merge a tier upward or keep spending/holding it. Each shares the
   exact same shape as `tickComputeNodeConversion`: converts every complete group of
   `COMPUTE_MERGE_RATIO` of the input entity into 1 of the output entity in a single call, capped at
   whatever room remains under `COMPUTE_ENTITY_CAP` (10) on the output, leaving surplus input
   unconverted rather than overflowing the output past its cap — a same-reference no-op below one
   full group of 8, or once the output is already at cap. A "Merge" button for each of these 8 pairs
   is always shown on `ComputePage` (never hidden, only disabled below `COMPUTE_MERGE_RATIO` held —
   see issue #316), staying available regardless of whether auto-merge (below) is also unlocked for
   that pair. Nothing yet spends a Megacomputer — it's the top of the chain today (see issue #280's
   "Out of scope"; no gameplay *effect* from holding any of these nine entities beyond Compute
   Boost's own spend of a Core is implemented yet).

   **Auto-merge automation** (see issue #316): each of the 8 manual merges above can be permanently
   automated, one tier boundary at a time. `enableAutoMergeNodesIntoCluster`/
   `enableAutoMergeClustersIntoNetwork`/`enableAutoMergeNetworksIntoGrid`/
   `enableAutoMergeGridsIntoFabric`/`enableAutoMergeFabricsIntoCloud`/
   ...
   `enableAutoMergeNodesIntoCluster` spends 10 Clusters — to permanently flip a matching boolean flag
   (`intro.autoMergeNodesIntoCluster`/…, all PERMANENT, defaulting `false`, carried over every real
   Prestige like every other compute-ladder field). Once flipped, `tickAutoMergeNodesIntoCluster`/…
   (each built off a shared `tickAutoMerge` factory) fires the identical `mergeComputeEntities` merge
   automatically every tick — but ONLY once the INPUT entity is completely full
   (`COMPUTE_ENTITY_CAP`, 10, not the manual button's own `COMPUTE_MERGE_RATIO`, 8) — a deliberately
   stricter bar so automation only ever mops up an entity the player has let cap out, never
   preempting a more efficient manual merge at 8. All 8 `tickAutoMerge*` functions are called from
   `tickGame` right after `tickComputeNodeConversion`, lowest tier first, reduced through in a row so
   a single tick can cascade automation all the way up the chain if every step along the way happens
   to be both unlocked and full. The manual merge button stays available and unaffected either way —
   auto-merge is additive automation, not a replacement for the manual action.
   ...
   `activateComputeBoost` and drained by `tickComputeNodeConversion`, so a player who spends a
...
10. **Compute Boost** (`intro.computeBoostType`/`computeBoostStacks`/`computeBoostRemainingSeconds`,
   ...
   first mechanic that actually spends Compute Cores. `activateComputeBoost(boostType)`, gated by
   `canActivateComputeBoost(state, boostType)`, always spends exactly 1 Compute Core regardless of
   preset — the preset (`COMPUTE_BOOST_PRESETS` in `layers.js`: `burst` ×16/1 minute, `standard`
   ×4/10 minutes, `sustain` ×2/1 hour) only picks the strength/duration tradeoff. The multiplier
   (`getComputeBoostMultiplier(intro)`) applies to "the base production tier of each screen"
   simultaneously — Memory's own passive production (`tickIntroProduction`, folded into `bitsToAdd`)
   and `tier01`'s production specifically (`tickGame`'s per-tier loop, where every other tier's own
   `computeBoostMultiplier` stays 1). Only one PRESET TYPE may be active at a time (a different type
   is blocked while one is running), but the SAME type may be activated again while already active,
   stacking `computeBoostStacks` up to `COMPUTE_BOOST_MAX_STACKS` (10) times to extend
   ...
   Only the UNUSED quantity is ever reclaimable, one at a time: `reclaimComputeBoost` (gated by
   `canReclaimComputeBoost`, true whenever any boost is active) is the exact inverse of one
   `activateComputeBoost` call — refunds 1 Compute Core (capped at `COMPUTE_ENTITY_CAP`, in case
   more were earned meanwhile) and subtracts that preset's own `durationSeconds` back out of
   ...
   "active" boost around. Activation happens in `ComputePage`'s Boost row — the available Core count
   and 3 small icon preset buttons share one row — but the active-boost status itself (effect,
   countdown, stack count, and the Reclaim control whenever `canReclaimComputeBoost`) renders at the
   TOP of the page, right after the header, so it stays visible regardless of what else is on
   screen; `MainPage` shows a matching read-only status line while a boost is active, since its
   effect reaches `tier01` there too. Ranked fourth in the forced priority order (below Storage Bank Fill/Bandwidth/Storage
...
Storage's Build button (and its per-size summary chip row) stays on ByteFoundryPage itself; only the
...
page" sections for the render-level layout. (No auto-redeem pause/resume control renders currently
on either page — see the "Redeeming" paragraph above.)
...
```js
{
  ...
  intro: {                                                // the Byte Foundry screen's own state — see "Byte
    ...
    computeNodes: 0,                                      // PERMANENT, capped at COMPUTE_ENTITY_CAP (10).
                                                          ...
                                                          // which are then spent. Also the input to
                                                          // mergeComputeNodesIntoCluster below (manual, not tick-driven)
    ...
    autoMergeNodesIntoCluster: false,                     // PERMANENT — each flipped once by the matching
    autoMergeClustersIntoNetwork: false,                  // enableAutoMerge* action (sacrifices ALL 10 held units
    autoMergeNetworksIntoGrid: false,                     // of that merge's OWN output entity), permanently
    autoMergeGridsIntoFabric: false,                      // enabling tickAutoMerge* to also fire that merge on
    autoMergeFabricsIntoCloud: false,                     // tick whenever its input entity is completely full
    autoMergeCloudsIntoDatacenter: false,                 // (COMPUTE_ENTITY_CAP, 10 — not the manual button's own
    autoMergeDatacentersIntoSupercomputer: false,         // COMPUTE_MERGE_RATIO, 8). The manual merge button stays
    autoMergeSupercomputersIntoMegacomputer: false,       // available either way — see issue #316
...
| `formatStorageSize` | `bits → string` | Byte Foundry Storage: formats a bank *size* using tier01's own per-unit level-cost scale — 1000 bits is "1 KB" ("KiloBits"), a completely separate scale from Memory's Byte-based one (`formatBitsInNearestUnit`, which every actually-spent bit amount renders through instead, including the transfer block's own dynamic cost, even though `getIntroKilobyteConversionCost` is the same underlying value/scale as a bank size). Reuses `TIER_DEFINITIONS`' KB..QB symbols. Shared by ByteFoundryPage (Build button label/per-size summary chips) and StoragePage (per-size bank labels) |
| `getStorageSizesToShow` | `state → number[]` | Byte Foundry Storage: every bank size worth showing, ascending — every size ever built (`intro.storageBanksBuiltTotal`), any size still held (`intro.storageBanks`, covers a migrated pre-ladder save missing a matching built-total entry), plus whatever `getStorageBankSize` currently offers (even at 0 built, so its row/goal is visible before the first one is banked). Shared by ByteFoundryPage's own brief per-size summary chip row and StoragePage's fuller per-size squares rows |
...
| `tickComputeCoreConversion` | `state → state` | Byte Foundry Compute Cores: no-op (same reference) unless `isComputeCoreConversionUnlocked` AND `intro.autoClaimCoreEnabled` (see `enableAutoClaimCore` below — added in issue #316; before that flag exists/is set, this is always a no-op, and Memory→Core conversion requires the manual `claimComputeCore` action instead). Delegates to the shared `mintComputeCoreIfReady` helper (see `claimComputeCore`'s own row for what it does). Called from `tickGame` right after `tickStorageAutoFill` and before `tickIntroAutoInvest` — the same "first claim on Memory" priority `tickStorageAutoFill` itself already has over ordinary Kilobyte conversion. Bypasses `isProductionFrozen`, same posture as every other Byte Foundry mechanic |
...
| `tickComputeNodeConversion` | `state → state` | Byte Foundry Compute Cores: same-reference no-op below `COMPUTE_CORES_PER_NODE` (8) Cores, or once `intro.computeNodes` is already at `COMPUTE_ENTITY_CAP` (10); otherwise converts complete groups of 8 into Nodes, capped at however many Nodes still fit under `COMPUTE_ENTITY_CAP` (`roomForNodes`) — a Core surplus beyond what fits is left unconverted in `intro.computeCores` rather than overflowing `computeNodes` past the cap. Only ever touches the live `computeCores` balance, never `computeCoresEverEarned` (that lifetime counter is only ever incremented by `tickComputeCoreConversion`, above). Called from `tickGame` right after `tickComputeCoreConversion`, so freshly-minted Cores convert the same tick they're earned |
| `mergeComputeNodesIntoCluster` | `state → state` | ComputePage merge chain (issue #280): player-triggered only, never called from `tickGame`. Same shape as `tickComputeNodeConversion` above but for `computeNodes → computeClusters` at `COMPUTE_MERGE_RATIO` (8): same-reference no-op below one full group of 8 `computeNodes`, or once `computeClusters` is already at `COMPUTE_ENTITY_CAP` (10); otherwise converts every complete group into `computeClusters`, capped at remaining room, leaving surplus `computeNodes` unconverted. Built off the shared `mergeComputeEntities(inputField, outputField)` factory, same as every merge function below |
| `mergeComputeClustersIntoNetwork` | `state → state` | ComputePage merge chain: identical shape to `mergeComputeNodesIntoCluster` above, one tier up — `computeClusters → computeNetworks` |
| `mergeComputeNetworksIntoGrid` | `state → state` | ComputePage merge chain: identical shape again — `computeNetworks → computeGrids` |
| `mergeComputeGridsIntoFabric` | `state → state` | ComputePage merge chain: identical shape again — `computeGrids → computeFabrics` |
| `mergeComputeFabricsIntoCloud` | `state → state` | ComputePage merge chain: identical shape again — `computeFabrics → computeClouds` |
| `mergeComputeCloudsIntoDatacenter` | `state → state` | ComputePage merge chain: identical shape again — `computeClouds → computeDatacenters` |
| `mergeComputeDatacentersIntoSupercomputer` | `state → state` | ComputePage merge chain: identical shape again — `computeDatacenters → computeSupercomputers` |
| `mergeComputeSupercomputersIntoMegacomputer` | `state → state` | ComputePage merge chain: identical shape again, the top of the chain — `computeSupercomputers → computeMegacomputers`. Nothing spends a Megacomputer yet |
| `tickAutoMergeNodesIntoCluster` / `tickAutoMergeClustersIntoNetwork` / `tickAutoMergeNetworksIntoGrid` / `tickAutoMergeGridsIntoFabric` / `tickAutoMergeFabricsIntoCloud` / `tickAutoMergeCloudsIntoDatacenter` / `tickAutoMergeDatacentersIntoSupercomputer` / `tickAutoMergeSupercomputersIntoMegacomputer` | `state → state` | Auto-merge automation (issue #316), each built off a shared `tickAutoMerge(inputField, outputField, autoFlagField)` factory: no-op unless the matching `intro.autoMerge*` flag is set; otherwise fires the identical `mergeComputeEntities` merge, but ONLY once the input entity is completely full (`COMPUTE_ENTITY_CAP`, 10 — not the manual button's own `COMPUTE_MERGE_RATIO`, 8). Called from `tickGame` right after `tickComputeNodeConversion`, lowest tier first (`AUTO_MERGE_TICKERS`), reduced through in order so one tick can cascade automation up multiple tiers in a row |
| `enableAutoMergeNodesIntoCluster` / `enableAutoMergeClustersIntoNetwork` / `enableAutoMergeNetworksIntoGrid` / `enableAutoMergeGridsIntoFabric` / `enableAutoMergeFabricsIntoCloud` / `enableAutoMergeCloudsIntoDatacenter` / `enableAutoMergeDatacentersIntoSupercomputer` / `enableAutoMergeSupercomputersIntoMegacomputer` | `state → state` | Auto-merge automation (issue #316), each built off a shared `enableAutoMerge(outputField, autoFlagField)` factory: no-op below the matching `isAutoMerge*UnlockAvailable` gate; otherwise sacrifices ALL `COMPUTE_ENTITY_CAP` (10) currently-held units of that merge's own OUTPUT entity and permanently sets the matching `intro.autoMerge*` flag true |
| `isAutoMergeNodesIntoClusterUnlockAvailable` / `isAutoMergeClustersIntoNetworkUnlockAvailable` / `isAutoMergeNetworksIntoGridUnlockAvailable` / `isAutoMergeGridsIntoFabricUnlockAvailable` / `isAutoMergeFabricsIntoCloudUnlockAvailable` / `isAutoMergeCloudsIntoDatacenterUnlockAvailable` / `isAutoMergeDatacentersIntoSupercomputerUnlockAvailable` / `isAutoMergeSupercomputersIntoMegacomputerUnlockAvailable` | `state → bool` | Whether the matching `enableAutoMerge*` action would do anything: `COMPUTE_ENTITY_CAP` (10) of the OUTPUT entity held AND the matching `autoMerge*` flag isn't already true |
...
| `reclaimComputeBoost` | `state → state` | Byte Foundry Compute Boost (issue #316): no-op below `canReclaimComputeBoost`'s own gate; otherwise the exact inverse of one `activateComputeBoost` call — refunds 1 `intro.computeCores` (capped at `COMPUTE_ENTITY_CAP`, in case more were earned while the boost was running) and subtracts the active preset's own `durationSeconds` back out of `computeBoostRemainingSeconds` (floored at 0), decrementing `computeBoostStacks` by 1; clears the boost fully back to inactive (`type: null`, `stacks: 0`, `remaining: 0`) once the last stack is reclaimed rather than leaving a 0-stack "active" boost around |
...
- `COMPUTE_ENTITY_CAP = 10` — Byte Foundry Compute Cores: maximum permanent balance of any compute-ladder entity (`computeCores`/`computeNodes`/`computeClusters`/`computeNetworks`/`computeGrids`/`computeFabrics`/`computeClouds`/`computeDatacenters`/`computeSupercomputers`/`computeMegacomputers`) — see `tickComputeCoreConversion`/`tickComputeNodeConversion`/every `mergeCompute*Into*` function
- `COMPUTE_MERGE_RATIO = 8` — ComputePage merge chain (issue #280): how many of one compute-ladder entity merge into 1 of the next tier up (Core → Node → Cluster → Network → Grid → Fabric → Cloud → Datacenter → Supercomputer → Megacomputer) — same ratio as `COMPUTE_CORES_PER_NODE` above, but a separate constant since this chain is player-triggered only, never automatic on tick — see `mergeComputeNodesIntoCluster`/`mergeComputeClustersIntoNetwork`/`mergeComputeNetworksIntoGrid`/`mergeComputeGridsIntoFabric`/`mergeComputeFabricsIntoCloud`/`mergeComputeCloudsIntoDatacenter`/`mergeComputeDatacentersIntoSupercomputer`/`mergeComputeSupercomputersIntoMegacomputer`
