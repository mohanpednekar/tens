...
| `formatStorageSize` | `bits → string` | Byte Foundry Storage: formats a bank size (or the transfer block's own dynamic cost) using tier01's own per-unit level-cost scale — 1000 bits is "1 KB" ("KiloBits"), a completely separate scale from Memory's Byte-based one (`formatBitsInNearestUnit`). Reuses `TIER_DEFINITIONS`' KB..QB symbols. Shared by ByteFoundryPage (Build button/summary chips/transfer blocks) and StoragePage (per-size bank labels) |
