...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeCoresEverEarned`/`intro.computeNodes`,
   ...
   **Merging Cores upward** (`intro.computeClusters`/`computeNetworks`/`computeGrids`, all
   PERMANENT like `computeCores`/`computeNodes` above; see issue #280) extends the same ladder
   three tiers further: 8 Compute Nodes → 1 Compute Cluster, 8 Compute Clusters → 1 Compute
   Network, 8 Compute Networks → 1 Compute Grid (`COMPUTE_MERGE_RATIO = 8`, the same ratio as
   `COMPUTE_CORES_PER_NODE` above, but a separate constant since this chain is conceptually
   distinct). Unlike Core→Node above, **none of these three merges ever fire automatically on
   tick** — each is a manual, player-clicked action (`mergeComputeNodesIntoCluster`/
   `mergeComputeClustersIntoNetwork`/`mergeComputeNetworksIntoGrid` in `engine.js`), so the player
   decides whether to merge a tier upward or keep spending/holding it. Each shares the exact same
   shape as `tickComputeNodeConversion`: converts every complete group of `COMPUTE_MERGE_RATIO` of
   the input entity into 1 of the output entity in a single call, capped at whatever room remains
   under `COMPUTE_ENTITY_CAP` (10) on the output, leaving surplus input unconverted rather than
   overflowing the output past its cap — a same-reference no-op below one full group of 8, or once
   the output is already at cap. Nothing yet spends a Compute Grid — it's the top of the chain
   today (see issue #280's "Out of scope"; no gameplay *effect* from holding any of these five
   entities beyond Compute Boost's own spend of a Core is implemented yet).
   ...
   counters themselves), gates the merge-chain section of `ComputePage` itself (Clusters/Networks/
   Grids counters + the three merge buttons) — the page as a whole already reveals earlier, via
...
```js
{
  ...
  intro: {                                                // the Byte Foundry screen's own state — see "Byte
    ...
    computeGrids: 0,                                      // PERMANENT, capped at COMPUTE_ENTITY_CAP (10).
                                                          ...
                                                          // computeNetworks -> 1). Top of the merge chain today —
                                                          // nothing spends a Grid yet (see issue #280)
...
| `mergeComputeNetworksIntoGrid` | `state → state` | ComputePage merge chain: identical shape again, the top of the chain — `computeNetworks → computeGrids`. Nothing spends a Grid yet |
...
- `COMPUTE_ENTITY_CAP = 10` — Byte Foundry Compute Cores: maximum permanent balance of any compute-ladder entity (`computeCores`/`computeNodes`/`computeClusters`/`computeNetworks`/`computeGrids`) — see `tickComputeCoreConversion`/`tickComputeNodeConversion`/`mergeComputeNodesIntoCluster`/`mergeComputeClustersIntoNetwork`/`mergeComputeNetworksIntoGrid`
- `COMPUTE_MERGE_RATIO = 8` — ComputePage merge chain (issue #280): how many of one compute-ladder entity merge into 1 of the next tier up (Node → Cluster → Network → Grid) — same ratio as `COMPUTE_CORES_PER_NODE` above, but a separate constant since this chain is player-triggered only, never automatic on tick — see `mergeComputeNodesIntoCluster`/`mergeComputeClustersIntoNetwork`/`mergeComputeNetworksIntoGrid`
