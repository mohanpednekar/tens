...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeCoresEverEarned`/`intro.computeNodes`,
   ...
   entirely. Once `capacity` reaches `INTRO_COMPUTE_CORE_UNLOCK_CAPACITY` (2,097,152 bits, "256 KiB"
   in Memory's own binary display scale — half of `INTRO_CAPACITY_CAP_BITS`, one Sacrifice doubling
   short of pool 1's hard cap — `isComputeCoreConversionUnlocked`), a full Memory
   ...
   Sacrifice for 2x Capacity already has (step 4 above) — and always grants exactly 1 Core per
