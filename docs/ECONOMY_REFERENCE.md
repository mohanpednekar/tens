...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeCoresEverEarned`/`intro.computeNodes`,
   ...
   **Once a boundary's auto-merge is unlocked, merging it fully transitions to a timed RESERVE POOL**
   ...
   (`getComputeMergeDurationSeconds` — live chain: each boundary is ×10 the previous layer’s
   duration by default, or ×5 once that boundary’s sequential duration upgrade is claimed; Core→Node
   starts at `COMPUTE_MERGE_BASE_DURATION_SECONDS` = 1s when unupgraded, matching the convenience
   snapshot `COMPUTE_MERGE_DURATIONS_SECONDS` = 1s / 10s / 100s / … / 1e8 s; see issues #370/#367).
...
- `COMPUTE_MERGE_BASE_DURATION_SECONDS = 1` — Core → Node duration when unupgraded; live durations come from `getComputeMergeDurationSeconds` (issue #370/#367)
- `COMPUTE_MERGE_STEP_MULTIPLIER = 10` / `COMPUTE_MERGE_STEP_MULTIPLIER_UPGRADED = 5` — each boundary is this many times the previous layer’s duration; a sequential upgrade (issue #367) switches that boundary’s step to ×5
- `COMPUTE_MERGE_DURATIONS_SECONDS = [1, 10, 100, 1000, 10000, 100000, 1000000, 10000000, 100000000]` — convenience snapshot of the unupgraded chain — see `getComputeMergeDurationSeconds`/`startComputeMergeReserve`/`tickComputeMergeBoundary`
- `COMPUTE_MERGE_DURATION_UPGRADE_COUNT = 9` — sequential one-time duration-step upgrades (one per merge boundary)
