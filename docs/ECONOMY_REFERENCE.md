...
`createInitialGameState().intro` (Memory → 0, Capacity → `INTRO_STARTING_CAPACITY`, Disks/Storage
and every Compute ladder entity / boost / auto-claim / auto-merge unlock / reveal latch wiped),
then **auto-restores** non-Capacity Foundry upgrades capped at the pre-reset highs: Combine
(`byteCreated`) and Invest / Bandwidth (`tickSpeedSeconds` / `productionMultiplier` /
`productionMilestoneTier` / `productionMilestoneTierClaims`). Capacity itself is never restored —
that is the point of the action. `intro.mainGameUnlocked` is preserved when already true. Every
...
| `resetByteFoundry` | `state → state` | Settings → Danger zone: fresh `intro` (Memory/Capacity/Disks/Compute wiped), then auto-restores Combine + Invest / Bandwidth fields capped at pre-reset highs. Capacity is never restored. Preserves `mainGameUnlocked` when already true. Leaves every non-`intro` field (Tiers, Prestige, automations, …) untouched |
