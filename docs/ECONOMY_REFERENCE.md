...
| `pickIntroCapacityMilestone` | `state → state` | **Upgrade Data Stream** action: drains a full Buffer (cost = current capacity) and doubles `intro.capacity` (plain `×2`, unclamped) via `upgradePoolCapacity`; Speed is derived from the new capacity, never purchased — see `getDataStreamSpeedBytesPerSecond`; below a full Buffer but at ≥ `INTRO_CAPACITY_UPGRADE_ARM_FRACTION` (99%) it instead arms the upgrade (`queueIntroCapacityUpgrade`), pausing all Data Stream outflow (`isDataStreamOutflowPaused` — `tickPoolBufferFill` incl. Data Lake overflow, `tickIntroAutoInvest`) until `tickQueuedCapacityUpgrade` fires it |
| `isMemoryCapacityUpgradeArmable` | `state → bool` | Upgrade Data Stream can be armed: byte combined, not already armed, not at cap, `intro.bits ≥ 99%` of capacity |
