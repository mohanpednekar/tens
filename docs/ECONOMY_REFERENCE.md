...
5. **Invest for Double Production** (`pickIntroProductionMilestone`) runs on its own **independent
   ...
   exactly `getIntroProductionMilestoneMaxClaims(t)` = **1** claim (an earlier version granted 2 for
   the three tiers below 1000 Bytes' worth before this tightened to 1 across the board, matching
   Sacrifice's own single-shot posture — see `docs/DESIGN_HISTORY.md`), tracked by
...
9. **Compute Cores/Nodes** (`intro.computeCores`/`intro.computeNodes`, both PERMANENT, carried over
   every real Prestige exactly like the Byte generator/Storage above) — once EVERY Storage bank
   size the ladder ever offers (now capped, see step 8 above) has `STORAGE_BANK_LADDER_CAP` (10)
   banks currently FULL (`isComputeCoreConversionReady`, checking `getComputeCoreStorageSizes()` —
   a small, fixed 3-size set: 1 KB, 10 KB, 1 MB, since `getStorageBankSize` no longer advances past
   `STORAGE_BANK_LADDER_MAX_SIZE`) **and** Memory itself is also full (`bits >= capacity`), Memory
   automatically converts into Compute Cores instead of idling (`tickComputeCoreConversion`) —
   `COMPUTE_CORE_MEMORY_COST` (80,000,000 bits, "10 MB" in Memory's OWN B/KB/MB display scale — a
   deliberately different "MB" convention from the Storage-ladder one above, see `layers.js`) per
   Core, spending only whole Cores' worth of bits and banking any remainder (there normally is
   none, since `COMPUTE_CORE_MEMORY_COST` is itself one of the Sacrifice capacity ladder's own
   values — the 8th step, 8 × 10⁷). Runs every tick (`tickGame`) right after `tickStorageAutoFill`
   and before `tickIntroAutoInvest`, so it claims Memory ahead of ordinary Kilobyte conversion once
   ready — the same "first claim" priority `tickStorageAutoFill` itself already has over
   `tickIntroAutoInvest`. Immediately after, every complete group of `COMPUTE_CORES_PER_NODE` (8)
   Compute Cores converts into 1 Compute Node the same tick (`tickComputeNodeConversion`), banking
   any remainder below that. Both bypass `isProductionFrozen`, same posture as every other Byte
   Foundry mechanic (a separate currency pool, not `resources.base`). Pure counters today — no
   gameplay effect yet, a foundation for a later mechanic to spend them on. `ByteFoundryPage` shows
   both counts in a small "Compute" section once Storage itself is revealed
...
```js
{
  ...
  intro: {                                                // the Byte Foundry screen's own state — see "Byte
    ...
    computeCores: 0,                                      // PERMANENT. Auto-incremented by
                                                          // tickComputeCoreConversion once every Storage bank
                                                          // size the (capped) ladder offers is built AND full,
                                                          // and Memory itself is also full — see
                                                          // isComputeCoreConversionReady/COMPUTE_CORE_MEMORY_COST.
                                                          // A pure counter — no gameplay effect yet
...
| `getStorageBankSize` | `state → number` | Byte Foundry Storage: an independent ladder that walks tier01's own per-unit LEVEL COST sequence (`getTierCost(TIER_DEFINITIONS[0], level)` for level 1, 2, 3, …) rather than a synthetic ×10 progression, advancing to the next level's cost every time `STORAGE_BANK_LADDER_CAP` banks have ever been built at the current one (read from `intro.storageBanksBuiltTotal`, cumulative — never decremented by redeeming) — the size `buildStorageBank` currently builds at. Because `getCostEpochExponent`'s exponent sequence skips values, this skips sizes too (e.g. level 3 = 1,000,000, never 100,000). **Stops advancing once it reaches `STORAGE_BANK_LADDER_MAX_SIZE`** (1,000,000, "1 MB") — from then on this always returns that same capped size, however many banks have ever been built at it, so `getComputeCoreStorageSizes` can enumerate every size the ladder ever offers as a small, fixed set. Deliberately decoupled from tier01's own CURRENT level cost; see `isStorageBankRedeemable` for the separate check on whether a built bank is spendable once full |
...
| `getComputeCoreStorageSizes` | `() → number[]` | Byte Foundry Compute Cores: every Storage bank size the (now-capped) ladder ever offers — `[1000, 10000, 1000000]` — walking `getFirstTierCost` up to `STORAGE_BANK_LADDER_MAX_SIZE` inclusive. A small, fixed set now that `getStorageBankSize` no longer advances past that cap |
| `isComputeCoreConversionReady` | `state → bool` | Byte Foundry Compute Cores predicate (not a reducer): true once every size `getComputeCoreStorageSizes()` lists has `intro.storageBanks[size] >= STORAGE_BANK_LADDER_CAP` (currently FULL, which can never exceed built) **and** `intro.bits >= intro.capacity` (Memory itself full) |
| `tickComputeCoreConversion` | `state → state` | Byte Foundry Compute Cores: same-reference no-op unless `isComputeCoreConversionReady`, or if Memory can't afford even 1 Core; otherwise converts `floor(bits / COMPUTE_CORE_MEMORY_COST)` whole Cores, spending only that many Cores' worth of bits (banking any remainder) and adding to `intro.computeCores`. Called from `tickGame` right after `tickStorageAutoFill` and before `tickIntroAutoInvest` — the same "first claim on Memory" priority `tickStorageAutoFill` itself already has over ordinary Kilobyte conversion. Bypasses `isProductionFrozen`, same posture as every other Byte Foundry mechanic |
...
- `STORAGE_BANK_LADDER_MAX_SIZE = 1_000_000` — Byte Foundry Compute Cores: caps `getStorageBankSize` from advancing past this size (tier01's level-3 per-unit cost, "1 MB" in the Storage ladder's own naming convention — a different "MB" from `COMPUTE_CORE_MEMORY_COST`'s own below) — turns "every Storage size the ladder ever offers" into the small, fixed 3-size set `getComputeCoreStorageSizes` enumerates (1 KB, 10 KB, 1 MB)
- `COMPUTE_CORE_MEMORY_COST = 80_000_000` — Byte Foundry Compute Cores: Memory cost of 1 Compute Core (see `tickComputeCoreConversion`) — 10 MB in Memory's OWN B/KB/MB display scale (`BITS_PER_BYTE * 1000²` per "MB"), not `STORAGE_BANK_LADDER_MAX_SIZE`'s own "1 MB" convention. Not a coincidence that this is exactly the 8th step of the Sacrifice capacity ladder (8 × 10⁷) — a cycle Sacrificed up that far converts its entire full Memory balance into whole Compute Cores with nothing left over
