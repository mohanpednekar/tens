...
builds with a per-array always-full cache as fallback tier funding when no matching disk exists;
Smart autobuyers auto-release cache; disks always take priority) redeemable against any main-game tier whose current price matches)
