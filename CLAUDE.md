...
`owned × (however many whole tickspeed-seconds have accumulated) × getPrestigeProductionMultiplier(points)
× getPurchaseMilestoneMultiplier(purchased)` in one shot and keeps any leftover remainder banked for the
next tick — nothing is ever lost, just delayed. Since `elapsedSeconds` is always 1 in the running app
(`TICK_RATE_MS` is 1000ms) and during offline-progress replay (`applyOfflineProgress` also always calls
`tickGame(1, …)`, one simulated second at a time), a tier with tickspeed *N* simply skips producing for
*N-1* ticks, then delivers *N* seconds' worth on the *N*th.
