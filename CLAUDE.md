...
`AppNav` (Foundry → Boosters → Factory → Guide → More) — not a router (see "Architecture" below):
...
src/
  ...
  components/
    AppNav/index.jsx        ← fixed bottom bar: Foundry → Boosters → Factory → Guide → More
  ...
  App.jsx                   ← root component; owns the single `useIncrementalGame()` call (lifted up
                               ...
                               `AppNav` (Foundry → Boosters → Factory → Guide → More) and `AppMenu`
