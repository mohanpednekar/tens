...
Foundry gate; only Tiers stays progress-gated.
...
spec-vs-implementation checks / backlog planning / process improvement; ~1% Cursor Pro soft
quota), offset from the Claude twice-daily cron. See `docs/AUTOMATION.md`'s
...
- **Cursor (Pro quota):** soft guidance is roughly **~1% of Cursor Pro quota per session**
  (especially the overnight housekeeping/planning slot). Prefer one small coherent unit; file
  non-trivial findings instead of half-implementing. Not a hard limit.
...
4. **`ByteFoundryPage/index.jsx`** — the tap screen (see "Economy model" below), also a pure renderer
   ...
   bootstrap — a mandatory gate whenever `intro.mainGameUnlocked` is false (AppNav omits Tiers during
   ...
   releasing (Disk Fill's manual-release half → Tiers Bits only), and redeeming (Disk Fill itself;
