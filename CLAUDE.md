...
- **Variables** — not a hand-maintained field list: `DevModePage`'s `FieldNode` recursively walks
  ...
  click), nested under a collapsible `<details>` per object ancestor (so `intro.dataLakes.1.used`
...
- Where several near-identical automations/buttons exercise the exact same generic UI behavior (a
  ...
  convention.
- `yarn test` is green (1528 tests). The four core test files (`engine.test.js`, `layers.test.js`,
