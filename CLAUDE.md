...
src/
  ...
  pages/
    ...
    ComputePage/index.jsx   ← the Compute screen, split out of ByteFoundryPage (see "Economy model"
                               below) — Compute Core/Node counters + the 3 Boost preset buttons, plus
                               (once `intro.computeMergePageUnlocked`) the manual Compute Cores merge
                               chain (Node → Cluster → Network → Grid, see issue #280). Reached only
    ...
    MainPage/index.jsx      ← the game itself; compact one-line-per-tier layout, data-driven from
                               ...
                               props — `game` is the full `useIncrementalGame()`
...
down later never re-hides the reveal. This latch gates only the merge-chain *section* of
`ComputePage` (the same page Boost already lives on, reached the same way — see above): before it
flips, `ComputePage` shows just Cores/Nodes counts and Boost, same as before this mechanic existed;
once unlocked it also shows all five compute-ladder counters and the three merge buttons.
`computeClusters`/`computeNetworks`/`computeGrids`/`computeMergePageUnlocked` are all PERMANENT,
carried over unchanged by every real Prestige exactly like `computeCores`/`computeNodes` themselves.
...
- Where several near-identical automations/buttons exercise the exact same generic UI behavior (a
  ...
  convention.
- `yarn test` is green (978 tests). The four core test files (`engine.test.js`, `layers.test.js`,
...
- Specs live under `e2e/` (a sibling of `src/`, not inside it), named `*.e2e.js` — deliberately not
  ...
  (978, see "Testing" above) is unaffected by anything under `e2e/`.
