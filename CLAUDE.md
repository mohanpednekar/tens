...
- Where several near-identical automations/buttons exercise the exact same generic UI behavior (a
  ...
  convention.
- `yarn test` is green (PLACEHOLDER_COUNT tests). The four core test files (`engine.test.js`, `layers.test.js`,
...
- Specs live under `e2e/` (a sibling of `src/`, not inside it), named `*.e2e.js` — deliberately not
  ...
  (PLACEHOLDER_COUNT, see "Testing" above) is unaffected by anything under `e2e/`.
