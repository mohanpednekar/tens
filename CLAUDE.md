...
| `getTierCost` | `(tier, level, blockSize) → number` | Returns the PER-UNIT price: `Math.ceil(levelTotalCost / blockSize)`, where `levelTotalCost = baseCost * 10^(getCostEpochExponent(epoch) - 1)`, epoch = `level - 1` — each level multiplies `baseCost` by 10 raised to (that level's cost-epoch exponent − 1): 1, 2, 4, 7, 11, … for epochs 0, 1, 2, 3, 4, … `levelTotalCost` is the total price of completing an entire level (every purchase within it, summed) — unchanged from the function's own pre-split return value; every purchase within a level costs an identical, even `1/blockSize` share of it, so buying all `blockSize` units costs exactly `levelTotalCost` **while `blockSize` evenly divides it** — true by construction for every real tier at the default block size (`baseCost` is always a multiple of `DEFAULT_PURCHASE_BLOCK_SIZE`). The division is rounded UP (`Math.ceil`), never down: once `blockSize` grows past its default (see `getPurchaseBlockSize`) it generally stops dividing evenly, and rounding down would either produce a fractional resource amount (violating the integer invariant) or, worse, round a real positive cost down to exactly 0 once `levelTotalCost < blockSize` (an infinite-free-purchase exploit) — rounding up instead means a full block can cost up to `blockSize - 1` more than `levelTotalCost` in that case, a minor, safe overcharge. Takes the tier's current LEVEL directly (`state.purchaseLevels[tierId]`), not a lifetime purchased count. See `docs/DESIGN_HISTORY.md` for why this multiplier form (and its triangular-number sequence, superseding an earlier Fibonacci one) was adopted over a literal `baseCost^exponent`, and for why the per-unit/level-total split (and its rounding direction) was introduced. Deep epochs still eventually overflow to `Infinity`, which is safe — an infinite cost is simply never affordable |
...
- `yarn test` is green (516 tests). All four test files assert against the current tier/resource id scheme
