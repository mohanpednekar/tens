...
2. **`useIncrementalGame.js`** — the only place holding React state. Owns the `setInterval` tick timer
   ...
   to the current level's cost-block boundary (`PURCHASE_BLOCK_SIZE` units, see "Economy model" below),
   via a `BUY_QUANTITY` constant (sourced from the shared `PURCHASE_BLOCK_SIZE` export) passed into
   `tickGame` as `autobuyerBatchSize` and into `actions.buyTierQuantity` (this replaced a
...
- **Owned vs. level.** `Owned` (current amount, drives production) is its own figure. `Purchased`
  (lifetime buy count, drives cost scaling and — every completed level — a production doubling via
  `getPurchaseMilestoneMultiplier`) has no separate cell: a tier's **level** (`getTierLevel(purchased)`
  in `engine.js`, 1-indexed — completing a level means buying `PURCHASE_BLOCK_SIZE` pieces of it, see
  "Economy model" below) shows on the Buy button's visible text as `Lv.{level} ({progress}/{PURCHASE_BLOCK_SIZE})`
  (e.g. `Lv.4 (5/8)` — the tier's current level and how many of that level's pieces are already bought)
  inside `ButtonIcon` alongside the 🛒 glyph, pinned immediately next to the icon rather than centered —
  this keeps the level text starting at the same x position across every tier row regardless of the
  cost string's length. The `aria-label` carries a `(level N, X of PURCHASE_BLOCK_SIZE purchased)`
  suffix in words instead. State field names (`state.purchased`, `getTierPurchasedCount`,
  `getPurchaseMilestoneMultiplier`) are unchanged; `getTierLevel` is a new derived accessor, and "level"
  is the player-facing term for its result (not a synonym for raw `purchased` the way it used to be).
...
- **Buy button.** Manual Buy always grabs as many units as are currently affordable up to the current
  level's `PURCHASE_BLOCK_SIZE`-unit cost-block boundary (`getTierAffordableQuantity`/
  `buyTierQuantity`) — no player-facing batch-size control. Renders its cost-block progress as an
...
Whenever the **last tier**'s currently-owned count is >= `PURCHASE_BLOCK_SIZE` (a full level, see
"Economy model" below; `isLastTierTickspeedXpUnlocked`, see "The last tier's XP-funded tickspeed"
below), this Money-funded `UpgradeButton` is replaced — in the same
...
shows up indirectly via the `⚙ +N%` badge and the tickspeed button's own tooltip.
...
generator, bought with Bits to produce more Bits — completing the byte-scale theme (1 Byte = 8 Bits;
...
A tier unlocks once you own **≥ `PURCHASE_BLOCK_SIZE`** (a full level's worth, currently 8) of the tier
below it (`isTierUnlocked`); already-owned tiers stay unlocked even if the rule changes later, so old
saves stay playable. Beyond that live check,
...
view as its own `GlobalTickspeedCard` (see "MainPage reference" above) rather than on the PP
...
`PURCHASE_BLOCK_SIZE` owned** (a full level), its own Money-funded tickspeed multiplier (see "Tickspeed
multiplier" above) is replaced by an XP-funded one instead — the last tier has no
...
- `isLastTierTickspeedXpUnlocked(state) = owned[lastTierId] >= PURCHASE_BLOCK_SIZE` — a **live** check
  against the last tier's current owned count, matching the same threshold every other tier's own
  unlock condition uses (see `isTierUnlocked`), deliberately not a permanent latch: a Prestige/Speed Up
  resets the last tier's `owned` count to 0 along with every other tier's (see `prestigeGame`/
  `speedUpGame` below), and this mechanic disengages right along with it — reverting the last tier's
  row back to the normal Money-funded tickspeed button — until the player buys back up to
  `PURCHASE_BLOCK_SIZE`, which re-engages it. There is no separate stored "ever unlocked" flag for this
...
- `state.lastTierXpConsumed` (run-scoped global counter, default `0`) is the cumulative total XP ever
  ...
  so the bonus it drives survives the mechanic being temporarily disengaged (owned dropping below
  `PURCHASE_BLOCK_SIZE`) and later re-engaged within the same run, but not a Prestige/Speed Up.
...
- `buyTickspeedMultiplier(lastTierId)` is a no-op for as long as `isLastTierTickspeedXpUnlocked` holds
  — there's nothing for that button to do for the last tier while it does; `tickGame`'s per-tier
  tickspeed self-upgrade loop (see `tierTickspeedAutobuyer`) calling it automatically for the last tier
  is harmless but inert for the same reason. It resumes working normally the moment owned drops back
  below 10 (e.g. after a Prestige/Speed Up).
...
`buyTier`/`buyTickspeedMultiplier`/`buyAutobuyerUnlock`/`buySmartAutobuyer`/`buyAutoPrestige`/`buyGlobalTickspeedMultiplier`/`consumeXpForLastTierTickspeed`
...
— a full level of the last tier; once the player has prestiged once, always shown when not frozen). Its
Prestige button shows `✦ +{award} PP ·
...
PURCHASE_BLOCK_SIZE * (speedUpCount + 1)` — `speedUpGame` (`engine.js`) resets resources/owned/purchased (everything a fresh
...
directly, normally doubling every time it completes another level (`getTierLevel`, see
`getPurchaseMilestoneMultiplier` — every 10th such level is a bigger ×10 milestone instead) — the
same boundary where `getTierCost`'s cost-epoch exponent steps up (see `getTierCost`),
...
| `getTierCost` | `(tier, purchasedCount) → number` | `baseCost * 10^(getCostEpochExponent(epoch) - 1)`, epoch = `getTierLevel(purchased) - 1` — flat across each level (`PURCHASE_BLOCK_SIZE` purchases); each level multiplies `baseCost` by 10 raised to (that level's cost-epoch exponent − 1): 1, 2, 4, 7, 11, … for epochs 0, 1, 2, 3, 4, … See `docs/DESIGN_HISTORY.md` for why this multiplier form (and its triangular-number sequence, superseding an earlier Fibonacci one) was adopted over a literal `baseCost^exponent`. Deep epochs still eventually overflow to `Infinity`, which is safe — an infinite cost is simply never affordable |
...
| `getTierLevel` | `purchased → number` | A tier's current level (1-indexed): `Math.floor(purchased / PURCHASE_BLOCK_SIZE) + 1` — purchased 0 through `PURCHASE_BLOCK_SIZE - 1` is level 1, the next `PURCHASE_BLOCK_SIZE` is level 2, and so on. The single source every level-dependent formula (`getTierCost`, `getTierBulkQuantity`, `getPurchaseMilestoneMultiplier`) and the UI read from, rather than each recomputing `purchased / PURCHASE_BLOCK_SIZE` independently |
| `getTierBulkQuantity` | `(tier, purchased, requestedQuantity) → number` | Caps a bulk purchase at the current level's cost-block boundary (via `getTierLevel`), so every unit bought is the same price |
...
| `buyTierQuantity` | `(tierId, quantity) → state → state` | Buys up to `quantity` units (capped at the cost-block boundary), stopping early if a unit becomes unaffordable; used both by the manual "Buy" button (always `quantity` `PURCHASE_BLOCK_SIZE`, see `useIncrementalGame`) and by `tickGame`'s autobuyer loop — the two purchase paths are identical, a tier's tickspeed multiplier level has no effect on how much a purchase costs or how many units it grants |
...
| `getPurchaseMilestoneMultiplier` | `purchased → number` | `levelsCompleted = getTierLevel(purchased) - 1`, `megaBlocks = floor(levelsCompleted/10)`, `regularBlocks = levelsCompleted - megaBlocks`; returns `PURCHASE_MILESTONE_MULTIPLIER_BASE ** regularBlocks * PURCHASE_MILESTONE_MEGA_MULTIPLIER_BASE ** megaBlocks` (`2`, `10`) — doubles a tier's own passive production at every completed level, the same boundary where `getTierCost`'s cost-epoch exponent steps up, **except** every 10th such level (i.e. every `PURCHASE_BLOCK_SIZE * 10`th lifetime purchase, currently the 80th) contributes a 10x factor instead of the regular 2x for that one level, compounding into the rest (e.g. purchased=80 → `2^9 * 10^1` = 5120, not `2^10` = 1024) — this "every 10th level" mega cadence is independent of `PURCHASE_BLOCK_SIZE` and stays a fixed 10 regardless of level size. Applies uniformly regardless of whether those purchases were manual or via an autobuyer |
...
| `getSpeedUpRequirement` | `speedUpCount → number` | `PURCHASE_BLOCK_SIZE * (speedUpCount + 1)` — how many lifetime purchases of the last tier the *next* Speed Up needs: `PURCHASE_BLOCK_SIZE` (currently 8) for the first activation, 2x that for the second, 3x for the third, … |
...
- `PURCHASE_BLOCK_SIZE = 8` — the shared purchase-block size defining a tier "level" (see `getTierLevel`): completing a level means buying this many pieces of it. Read by `getTierCost`'s cost epoch, `getTierBulkQuantity`'s bulk-purchase cap, `getPurchaseMilestoneMultiplier`'s doubling cadence, `BUY_QUANTITY` (`useIncrementalGame.js`), the smart-autobuyer bootstrap threshold, `getSpeedUpRequirement`'s per-cycle step, and `isTierUnlocked`/`isLastTierTickspeedXpUnlocked`'s owned-count thresholds — a single source rather than each hardcoding its own copy. Supersedes an old block size of 10 — see `docs/DESIGN_HISTORY.md`
...
- `PURCHASE_MILESTONE_MEGA_MULTIPLIER_BASE = 10` — every 10th level (i.e. every `PURCHASE_BLOCK_SIZE * 10`th lifetime purchase) uses this larger factor instead of `PURCHASE_MILESTONE_MULTIPLIER_BASE` for that one level, compounding into the rest (see `getPurchaseMilestoneMultiplier`) — this "every 10th level" cadence is independent of `PURCHASE_BLOCK_SIZE` and stays fixed at 10
...
- `LAST_TIER_XP_TICKSPEED_STEP = 0.01` — each XP consumed via `consumeXpForLastTierTickspeed` within the current run compounds another 1% into the last tier's own delivery frequency (see `getLastTierXpTickspeedMultiplier`) — the mechanic that replaces that tier's Money-funded tickspeed multiplier while it currently has >= `PURCHASE_BLOCK_SIZE` owned; both the XP spent and the bonus it drives reset to 0 on Prestige/Speed Up (see "The last tier's XP-funded tickspeed" above)
...
- `yarn test` is green (506 tests). All four test files assert against the current tier/resource id scheme
  ...
  migrated to `resources.base` on load (see `storage.js`'s `migrateState`).
