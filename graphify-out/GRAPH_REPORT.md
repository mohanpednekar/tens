...
### Community 0 - "Testing"
Cohesion: 0.17
Nodes (23): A Devin Review finding on the PR above: the target-stranded gate broke cross-tier-boundary write-cache chains — removed the "stranded" gate from write-cache entirely, A Devin Review pass on the idle-disk-liquidation removal found write-cache still consuming stranded disks, A further Devin Review finding on the same area: pausing a stranded write-cache merge still lost its progress to Prestige — fixed by making diskWriteCache/diskReadCacheFlush Prestige-permanent, A tenth finding: idle disk liquidation could starve a still-needed write-cache merge of its own source disks, An adversarial review pass on PR #603 caught the new cross-tier-boundary test asserting a false "would have failed under the prior fix" claim, An eleventh finding: the Data Lake overflow taper was sampled once per disk-completion segment, not truly continuous — making a single tick's own result depend on how it was split, Compute Boost base presets: fixing a total-extra-production ordering bug, Data Lake Boosters: spending real deposits, not a separate "used" ledger (+15 more)

### Community 1 - "MainPage/index.jsx"
Cohesion: 0.03
Nodes (58): Money, BalancesSentinel, BuyButton, BuyButtonCostLabel, BuyButtonIcon, BytePowerSegment, BytePowerSegmentFill, BytePowerSegments (+50 more)

### Community 2 - "Economy model"
Cohesion: 0.10
Nodes (22): Compute Boost: the first mechanic to spend Compute Cores, and a Sacrifice confirmation, Compute Boost tier scaling: 4× effect only, no duration enhancement (#363), Economy model, Prestige history: why PP replaced direct production doubling, Reintroducing the 1s-10s tickspeed ladder, Reset button history, `scaleUpGame`'s `highestMilestone` passthrough was a real bug, not a harmless asymmetry, Starting Money reduced from 10 to 1 (+14 more)
...
Cohesion: 0.06
Nodes (56): Ladder screen renamed back to Byte Factory (reverses #399/#431) — 2026-08-31, Sacrifice confirm: in-game dialog; Core warning only when unlocked, isAutoMergeCloudsIntoDatacenterUnlockAvailable(), isAutoMergeClustersIntoNetworkUnlockAvailable(), isAutoMergeCoresIntoNodeUnlockAvailable(), isAutoMergeDatacentersIntoSupercomputerUnlockAvailable(), isAutoMergeFabricsIntoCloudUnlockAvailable(), isAutoMergeGridsIntoFabricUnlockAvailable() (+48 more)

### Community 5 - "ComputePage/index.jsx"
Cohesion: 0.06
Nodes (57): Compute Boost: Reclaim and Forfeit made mutually exclusive — 2026-09-04, activateComputeBoost(), canActivateComputeBoost(), canForfeitComputeBoost(), canReclaimComputeBoost(), canStackComputeBoost(), forfeitComputeBoost(), getBiggestComputeTierWaitingOnMerge() (+49 more)

### Community 6 - "MainPage reference"
Cohesion: 0.14
Nodes (35): "0.xyz <unit>" fractions eliminated from every Byte/bit-denominated display, Data Stream balance: raw-bits fallback narrowed to self-sizing into a finer unit; Pool Bandwidth moved beside its title, Data Stream/pool balances skip their padded trailing zeros once full for more than a second, Fibonacci cost curve and 2-claims-for-the-first-three-Invest-tiers reinstated, this time deliberately, Pool 1 byte generator: binary Memory units, doubling capacity cap, ×4 Bandwidth ladder (#457, epic #456), Provision Disk button no longer previews progress before the player has ever clicked it, Provision Disk's idle label now shows "0/N" up front; write-cache collect sped up to 5x, MainPage reference (+27 more)
...
Cohesion: 0.06
Nodes (56): AUTO_MERGE_TICKERS, BIT_UNIT_SYMBOLS, combineIntroByte(), COMPUTE_MERGE_TIMER_FIELDS, currencyNumberFormatter, enableAutoMergeCloudsIntoDatacenter, enableAutoMergeClustersIntoNetwork, enableAutoMergeCoresIntoNode (+48 more)
...
### Community 10 - "buyBooster"
Cohesion: 0.23
Nodes (22): `ByteFoundryPage` pool layout, A ninth finding: a lake's escalating Booster cost could outgrow its own permanently-capped capacity, bricking it forever, Adversarial-review follow-up to the extended-cap/one-shot-conversion PR: a stray merge corruption, a real reserve-wipe bug, and a stuck-conversion bug — 2026-09-18, Auto-merge Booster progress display, a gradually-filling 18-slot extended cap, and one-shot Data Lake conversion replacing the persistent Auto/Manual toggle — 2026-09-17, Boosters UI revamp: buyBooster now pauses at COMPUTE_ENTITY_CAP; tier row buttons no longer clump left — 2026-09-17, Three more findings from a Devin bot review pass on the pool-overflow Data Lake rework PR: an overflow rate that asymptotically never completes, a lifetime-counter bug, and dropped legacy transfers, buyBooster(), enableAutoMerge() (+14 more)

### Community 11 - "Key engine functions (`src/game/engine.js`)"
Cohesion: 0.13
Nodes (32): seedState(), Era ascension and Eons — meta-prestige above Unbounded (#407 / #405), Tier autobuyer unlock/tier tickspeed autobuyer became free, prestige-count-milestone unlocks, Why the Prestige threshold became `GOOGOL * BITS_PER_BYTE`, not a round new number, Constants (`src/game/layers.js`), Key engine functions (`src/game/engine.js`), Prestige and the Googol freeze, applyAutobuyerMilestones() (+24 more)

### Community 12 - "getTierCost"
Cohesion: 0.20
Nodes (26): actMainBuys(), wouldAutobuyerStall(), Cost-epoch exponent sequence changed a third time: Fibonacci replaced with a linear-increment one, `getTierCost`'s division-based split was replaced by a fixed-price-times-blockSize model, `getTierCost` split into per-unit price vs. level-total price, Last tier's XP-funded tickspeed: from a permanent latch to a live owned >= 10 check, Purchase block size became a runtime-configurable, growing value — `getTierLevel` replaced by direct state tracking, Purchase level resized from 10 to 8, and the cost-epoch sequence changed from Fibonacci to triangular (+18 more)
...
Cohesion: 0.05
Nodes (41): COMPUTE_FLOPS_BOOST_RATE_PER_UNIT_PER_SEC, COMPUTE_FLOPS_FIRST_TIER_COST_PP, COMPUTE_FLOPS_REVEAL_PP, COMPUTE_FLOPS_TIER_DEFINITIONS, ERA_ELIGIBILITY_PP, PRESTIGE_THRESHOLD, TIER_DEFINITIONS, FlopsHero (+33 more)

### Community 15 - "Byte Foundry"
Cohesion: 0.34
Nodes (15): `DiskArrayRow/index.jsx`, A second Devin Review finding on the same PR: the level-1 cache fallback could spend cache out from under an in-flight read-cache flush, leaving it stuck for its whole remaining duration then producing no disk, Disk redemption: from price coincidence to a fixed one-to-one tier+level mapping, Storage funding rebuilt push→pull: the manual Redeem button and autobuyer-gated auto-redeem are gone (issue #571), Byte Foundry, getDataLakeSubSize(), getDataLakeTierIndex(), getDiskLadderStep() (+7 more)

### Community 16 - "createInitialGameState"
Cohesion: 0.09
Nodes (29): Accessibility, Added, Added, Added, Changed, Changed, Changed, Changed (+21 more)
...
Cohesion: 0.09
Nodes (37): Changed, Data Lake Boosters, take two: from a spendable balance to a live transfer pipe, Pool gauge's separate bottom-half Data Lake arc replaced with one dial that switches meaning once the buffer is full, Pool titles simplified to "<symbol> Pool"; each pool's Data Lake moved inside its own card, ActionButton, BareDivider, clampFraction(), DataLakePanel() (+29 more)
...
Cohesion: 0.04
Nodes (25): clearIntroCapacityUpgradeQueue(), eraseAllComputeTokens(), getComputeMergeDurationSeconds(), isAnyComputeMergeInFlight(), isLaterPoolProvisioningLocked(), startComputeMergeReserve(), startComputeMergeReserveAtBoundary(), eraEligibleState() (+17 more)

### Community 23 - "progressFill"
Cohesion: 0.67
Nodes (3): clampPercent(), progressFill(), PrestigeProgressTrack

### Community 24 - "defaultSlotName"
Cohesion: 0.83
Nodes (4): buildDefaultMeta(), coerceMeta(), defaultSlotName(), withSupporterSlots()

### Community 25 - "tokens.js"
Cohesion: 0.16
Nodes (12): GlobalStyle, getSystemThemeMode(), DEFAULT_MODE, font, MODES, motion, palette, radius (+4 more)
...
Cohesion: 0.12
Nodes (23): `AppNav/index.jsx`, Theming reference, App(), GATE_EXEMPT_PAGES, PageShell, resolveInitialThemeMode(), AppMenu(), Backdrop (+15 more)
...
Cohesion: 0.10
Nodes (40): PP Compute (Flops), 2024-05-24 - Bulk Purchase State Updates in React Incremental Game, 2024-05-25 - Replace O(N) while loop for Booster bulk purchases with O(1) mathematical formulation, 2024-06-25 - Replace O(N) cost epoch exponent calculation with O(1) mathematical equivalent, 2024-06-25 - Replace O(N) while loop for Compute Flops bulk purchases with O(1) loop equivalent calculation, 2024-07-28 - Replace O(N) attempts loop with O(1) batch processing for tickGame autobuyers, 2024-09-20 - Replace O(N) Compute Flops autobuyer with O(1) mathematical formulation, buyAutoPrestige() (+32 more)
...
Cohesion: 0.15
Nodes (39): Pool-local resets, actFoundry(), What it does, A fourth Codex round: the "absolute ceiling" clamp itself was too high, A live tap bonus could survive into the pool gauge's mode switch, breaking the "clean transition at 50%" claim, Bandwidth cap corrected to sqrt(Capacity in Bytes), not raw bits; Storage pools switched to SI display, Disk/Cache fill speeds tied to Memory bandwidth, not flat/hardcoded rates, Per-pool Memory buffers: a real intermediary reservoir between the Data Stream and Storage spending (+31 more)

### Community 33 - "MainPage"
Cohesion: 0.21
Nodes (23): Last tier's XP-funded tickspeed: from additive to multiplicative, Multiplier overflow safety: the switch to compounding needed a floor, Overclock, again: the standalone multiplier comes back, deliberately, plus a full requirement rework, Overclock: from a standalone multiplier to a Tickscale-upgrade step boost, Overclock, once more: back to folding into the Tickspeed multiplier's own step — now multiplicative and covering milestones too, Tier tickspeed upgrade reverted from +1% to +10% per level — 2026-09-14, Why the tick-progress ring was removed, Multiplier overflow safety (+15 more)

### Community 34 - "Disk arrays and Data Lakes moved from 10 disks per size to 9 + cache/buffer as the 10th unit"
Cohesion: 0.25
Nodes (16): A fifth and sixth Devin finding on the same PR: a one-tick lake-overflow lag, and a currency-destroying overshoot in fillDataLakeDisks it exposed, A fourth Devin finding on the same PR: the disk-square decomposition could strand real, spendable units with no square to show for them, A seventh finding: the pool gauge could display a nonzero incoming-overflow rate on an already-full lake, An eighth finding: a tick spanning more than one lake-disk completion reused the first disk's stale overflow rate for the rest, Disk arrays and Data Lakes moved from 10 disks per size to 9 + cache/buffer as the 10th unit, The multiplier bar moved below the balance, with its percent readout below the bar itself, applyDataLakeOverflow(), decomposeDataLakeUnits() (+8 more)
...
Cohesion: 0.17
Nodes (30): 2. Load the repo's invariants, actSoftResets(), When editing the simulation, `prestigeGame` wiped era/eons/hyperscalerCount/eonsUpgrades/Flops-autobuyer state on every ordinary Prestige (#626) — 2026-09-09, Speed Up renamed to Scale Up and redesigned from a last-tier-only gate to a per-tier unlock ladder — 2026-09-09, The Byte Foundry becomes a per-Prestige-cycle mechanic, not a one-time gate, Why Scale Up exists, and why its requirement escalates, Era ascension and Eons (#407) (+22 more)
...
Cohesion: 0.21
Nodes (18): 5. Authorization boundary, `OfflineProgressNotice/index.jsx`, Architecture / MainPage UI decisions, `PURCHASE_MILESTONE_MULTIPLIER_BASE` raised 1.1 → 1.25; a 2-vs-3-Overclock-claim "stretch/easy" retune was explored and dropped, The Byte generator becomes permanent — only "Memory" and the gate reset each Prestige, The transfer-block row looked permanently stuck — `tickIntroAutoInvest` waited for a whole batch instead of converting live, Multiplier outcomes are floored, Tickspeed multiplier (+10 more)
...
### Community 42 - "SettingsPage"
Cohesion: 0.20
Nodes (9): `AppMenu/index.jsx`, `ConfirmDialog/index.jsx`, `IncompatibleSaveNotice/index.jsx`, `Money/index.js`, Shared components reference, `StatCard/index.js`, buildClearSlotConfirmMessage(), buildEraseAllSavesConfirmMessage() (+1 more)
...
Cohesion: 0.07
Nodes (29): ActionsRow, BalanceSeparator, BalanceText, BarFillBase, BarFillBonus, BarFillLake, BarPercentLabel, BarRow (+21 more)
...
Cohesion: 0.17
Nodes (22): Adding a new tier, Architecture, Path aliases (`vite.config.js`), Project, Added, Architecture, Project, A third Codex round: invisible cache activity, a stale Fill tooltip, a Buy button hidden behind Scale Out, and an unclamped legacy-save buffer (+14 more)
...
### Community 63 - "ButtonContent"
Cohesion: 0.40
Nodes (5): Fixed, `Button/index.jsx`, ButtonContent(), OfflineProgressNotice(), formatOfflineDuration()
...
Cohesion: 0.11
Nodes (25): react, styled-components, Button, ButtonIcon, ButtonLabel, getGlowRgb(), hexToRgb(), NAMED_GLOW_RGB (+17 more)
...
Cohesion: 0.18
Nodes (39): Dev Mode, Security notes, Offline progress, getOfflineEffectiveSeconds(), PRESTIGE_UNBOUNDED_MIN_COUNT, clearAllSaveProgress(), clearDevGameState(), clearGameState() (+31 more)
...
- **Why does `main()` connect `main` to `Pool-local resets`, `MainPage`, `isProductionFrozen`, `MainPage reference`, `Automation workflows`, `buyBooster`, `createInitialGameState`, `DataLakePanel/index.jsx`, `bump-version.mjs`, `file-task-issue/SKILL.md`, `tickIntroAutoInvest`, `provisionDisk`, `CLAUDE.md`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `Pool-local resets` connect `Pool-local resets` to `Testing`, `ComputePage/index.jsx`, `MainPage reference`, `DiskArrayRow`, `buyBooster`, `Key engine functions (`src/game/engine.js`)`, `isMemoryCapacityAtCap`, `Byte Foundry`, `DataLakePanel/index.jsx`, `formatCurrency`, `clampNonNegative`, `CLAUDE.md`, `Disk arrays and Data Lakes moved from 10 disks per size to 9 + cache/buffer as the 10th unit`, `isProductionFrozen`, `tickGame`, `Data Lake unlock/capacity tied to real Storage progress; giant-circle CSS bug; Compute Boost reclaim floor — 2026-09-03`, `main`, `Architecture`, `tickIntroAutoInvest`, `provisionDisk`, `fillDataLakeManually`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `Design history & rationale` connect `provisionDisk` to `Testing`, `MainPage`, `Economy model`, `navAttention.js`, `ComputePage/index.jsx`, `isProductionFrozen`, `Automation workflows`, `tickGame`, `buyBooster`, `Tens`, `Data Lake unlock/capacity tied to real Storage progress; giant-circle CSS bug; Compute Boost reclaim floor — 2026-09-03`, `isMemoryCapacityAtCap`, `createInitialGameState`, `tickIntroAutoInvest`?**
