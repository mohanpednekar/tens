...
- **Dev Mode** — a local, dev-build-only sandbox (More → Dev Mode, rendered only when
  ...
  your real progress; quick-seed presets, quick numeric field edits (Bits, Bytes, Prestige Points,
  Prestige count, Eons, Era count, every tier's Owned), and a raw state-JSON editor that merges a
  partial object (only the fields you're changing) onto the current dev save.
