...
- **Storage**, its own labeled section on the Byte Foundry screen: bank a block of bits now (in KB/MB/GB/… sizes matching Kilobytes' own future per-unit level costs) to redeem later for a free Kilobyte, once Kilobytes' level actually reaches the price that block was built for. Building a bank costs 10x its own size in bits; each held denomination shows as a small chip (`⟨size⟩ ×⟨count⟩`) that becomes clickable/highlighted once its size is at or below Kilobytes' current level cost. Banks are never lost — redeem them by hand, or flip on the auto-redeem toggle to have every eligible bank spend itself automatically, no click needed.
