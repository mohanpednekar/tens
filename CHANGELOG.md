...
### Changed
- The main game page no longer shows any explanatory prose about how a mechanic works (the Tickspeed/Speed Up/Overclock cards' click-to-expand explanations, the Tier Autobuyers category's "How these controls work" panel, the Milestones page's two category descriptions, and the page header's tagline) — that content moved to the new Guide page (see Added, above). Card headings are now plain (no longer click-to-expand), and any number that reflects the current run's live state (like the Tickspeed card's current level/bonus) stays on the game page as before — Overclock's current per-level Tickspeed rate and the Milestones page's Tier Tickspeed Autobuyers start/step numbers now surface via a small `ⓘ` hover/tap badge on their heading instead.
