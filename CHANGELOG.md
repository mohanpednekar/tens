...
- The last tier's manual XP-consume tickspeed control (`🧬 {XP} XP`) moved out of that tier's own row and up to a new top-of-page card, "XP Tickspeed," which sits side by side with the (renamed, see below) "Tickspeed" card near the top of the Game view — both above the tier list, wrapping to stacked on narrow viewports. The last tier's row keeps a quick-access Speed Up button (`⏩ ×N`) in the grid slot the XP-consume control used to occupy, since reaching a full last-tier level is also typically when Speed Up becomes available; the row's own Auto Speed Up status badge moved alongside it. The underlying XP-funded tickspeed mechanic still runs automatically via the tier tickspeed autobuyer regardless of which control is used. The "Global Tickspeed Multiplier" card/label was also renamed to the more concise "Tickspeed" throughout (via an intermediate "Global Tickspeed").
