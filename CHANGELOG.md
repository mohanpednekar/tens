...
- **Storage** on the Byte Foundry screen: bank a block of bits now (in KB/MB/GB/… sizes matching Kilobytes' own future per-unit level costs) to redeem later for a free Kilobyte, once Kilobytes' level actually reaches the price that block was built for. Building a bank costs 10x its own size in bits; a held bank becomes clickable the instant its size matches Kilobytes' current level cost exactly. Banks are never lost — redeem them by hand, or flip on the new auto-redeem toggle to have a matching bank spend itself automatically, no click needed.
