...
### Added
- A new **Byte Foundry** pre-game screen, shown before the main game on every fresh save: tap to generate bits, combine your first 8 into a permanent Byte generator that then produces bits passively, and grow two independent tracks each time your bit balance fills — sacrifice it all for 10x capacity, or spend the current capacity's worth of bits (without needing to be full, and without losing the rest) to double your production rate. Once capacity reaches 1000 you can manually convert bits into Kilobytes (1000 bits per Kilobyte); once your balance reaches 8000, it auto-converts into 8 Kilobytes exactly once, transitioning you into the main game. Replaces the old self-producing Bytes tier as the game's actual bootstrap.
