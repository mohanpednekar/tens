...
- **Storage**, its own labeled section on the Byte Foundry screen: bank a block of bits now, in KB/MB/GB/… sizes matching Kilobytes' own current per-unit level cost (starting at 1 KB on a fresh save) — building a bank spends 10x its own size in bits and it's redeemable for a free Kilobyte right away, staying redeemable even after Kilobytes levels up further, so you can queue several banks ahead of a purchase burst at today's price and redeem them whenever convenient. Each held denomination shows as a small chip (`⟨size⟩ ×⟨count⟩`) that's clickable/highlighted once its size is at or below Kilobytes' current level cost. Banks are never lost — redeem them by hand, or flip on the auto-redeem toggle to have every eligible bank spend itself automatically, no click needed.
