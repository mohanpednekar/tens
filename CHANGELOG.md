...
- The Speed Up card now renders near the top of the Game view, side by side with the Global Tickspeed card (wrapping to stacked on narrow viewports), instead of as the last item after the tier list. The last tier's row no longer shows a manual XP-consume button (`🧬 {XP} XP`) once it's full — that grid slot now shows a quick-access Speed Up button instead, since reaching a full last-tier level is also typically when Speed Up becomes available. The underlying XP-funded tickspeed mechanic still runs automatically via the tier tickspeed autobuyer; only its manual trigger was removed. The "Global Tickspeed Multiplier" card/label was also renamed to the more concise "Tickspeed" throughout (via an intermediate "Global Tickspeed").
