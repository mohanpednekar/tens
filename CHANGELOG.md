...
- **Storage**, its own labeled section on the Byte Foundry screen: bank a block of bits now, in KB/MB/GB/… sizes, and redeem it later for a free Kilobyte once Kilobytes' own level cost catches up to that size. The buildable size follows its own ladder — starts at 1 KB, costing 10x that in bits, and only offers the next size up (10 KB, then 100 KB, …) once 10 banks have ever been built at the current one. Every size ever built shows as a row of up to 10 squares — already-redeemed (greyed out), currently held (clickable/highlighted once redeemable), and not-yet-built — reading left to right as one progress bar. Banks are never lost — redeem them by hand, or flip on the auto-redeem toggle to have every eligible bank above 1 KB spend itself automatically (1 KB banks always auto-redeem, toggle or not); either way, a given size only auto-redeems once per Prestige cycle, so further banks of that size need a manual click for the rest of the run. The section also shows Kilobytes' own current purchase-progress as a row of blocks, filling in regardless of whether a unit came from the main game's Buy button or from redeeming a Storage bank here.
