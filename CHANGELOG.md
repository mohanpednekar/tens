...
- Removed the per-tier unit-autobuyer pause/resume button from the Game view tier rows to reduce clutter — the row still shows a read-only `Active`/`Paused` badge, and the actual pause/resume control now lives on the PP Upgrades page's Tier Autobuyers category alongside the tier tickspeed autobuyer's own toggle.
