...
- Reaching the main game no longer requires filling Memory all the way to 8000 bits — converting your first 1000-bit chunk into a Kilobyte (a manual click) now unlocks it immediately. Further conversions, manual or via the existing "Memory fills to 8000 → auto-convert" convenience, keep working after that, sharing one running transfer budget capped at 8000 bits per Prestige cycle — once that's spent, no more transfers happen until your next Prestige reopens a fresh budget.
...
- The Byte Foundry's tap button shrinks to roughly half its original width (while staying just as tappable, and keeping its original aspect ratio) once passive production starts, since it becomes a secondary/backup action rather than the primary loop. The production-rate readout below 1 Byte/sec now shows an 8-block segmented bar alongside the "+N bits/sec" text; at/above 1 Byte/sec it switches to a "+N Byte(s)/sec" label instead. Every Byte Foundry action button (Tap/Combine/Sacrifice/Invest/Convert/Build Storage Bank) now shows its own fill progress toward affordability, matching the main game's Buy/Upgrade button convention, and the Invest button shows its own current cost. The bit balance is now labeled "Memory" on its own filling tile, alongside a second always-visible tracker showing your progress (in raw bits) toward the current 8000-bit transfer block — and, unlike before, it's now denominated in whatever unit actually fits (raw bits, then B/KB/MB/…/QB by 1000 each step) instead of staying in raw bits until a Byte generator exists, then Bytes forever after.
