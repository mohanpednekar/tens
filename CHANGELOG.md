...
- The bottom Prestige panel (Game view) no longer has its own Prestige button — the Prestige Points display at the top of the page already doubles as the Prestige button once available, so the bottom panel is now purely informational (still expandable for prestige count/unspent PP/production speed bonus/Auto-Prestige status).
