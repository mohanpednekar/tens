...
### Added
- A new **Byte Foundry** pre-game screen, shown before the main game on every fresh save: tap to generate bits, combine your first 8 into a permanent Byte generator that then produces bits passively, and grow two independent tracks each time your bit balance fills — sacrifice it all for 10x capacity, or invest in "double production" via its own independent cost ladder (1/10/100/1000/10000 Bytes, two claims per tier below 1000 Bytes, one claim per tier from 1000 Bytes on — no need to be full to claim). Once capacity reaches 1000 Bytes, a row of transfer blocks lets you convert bits into Kilobytes (1000 bits per block) — always all of them, for the whole cycle: only the leftmost not-yet-transferred block is clickable at a time, and the very first transfer unlocks the main game immediately, but blocks stay on screen once spent, greying out to show they're consumed rather than disappearing. Further transfers (clicking through blocks, or all at once via an auto-convenience once every remaining block is available) share one per-Prestige-cycle transfer budget, sized to match the Kilobyte tier's own current purchase block size (8 blocks — 8000 bits — at the start of a run, growing later). Replaces the old self-producing Bytes tier as the game's actual bootstrap.
...
- **Overclock**: a new soft-reset button, next to Speed Up below the tier list, requiring the last tier to reach level 10 (20 for the second activation, 30 for the third, …). Like Speed Up, it resets your tiers and resources (keeping unlocked autobuyers and Prestige Points) — but it also wipes Speed Up's own stacking production bonus back to zero, and in exchange permanently raises the (Money-funded) Tickspeed upgrade's own per-level rate by another 0.1 percentage points (1% → 1.1% → 1.2% → … per activation), rather than adding a separate multiplier — so its effect only shows up once you've bought at least one Tickspeed level. Unlike Speed Up's own bonus, Overclock's boost survives an ordinary Speed Up and only resets on a real Prestige.
- **Storage**, its own labeled section on the Byte Foundry screen — a genuine storage medium, not a one-shot purchase. Building a bank only constructs a permanent, empty container, in KB/MB/GB/… sizes matching one of Kilobytes' own real per-level costs (the ladder skips sizes Kilobytes itself skips, like 100 KB); the build cost is 10x that size in bytes. Memory then automatically fills any empty container over time, smallest size first, whatever's left over staying as ordinary Memory. Redeem a full bank for a free Kilobyte once Kilobytes' own level cost catches up to that size — redeeming empties it again, ready to be auto-filled and redeemed once more, so nothing is ever single-use. Every size ever built shows as a row of up to 10 squares — currently full (clickable/highlighted once redeemable), built-but-empty (waiting for Memory), and not-yet-built — reading left to right as one progress bar. Redeem by hand, or flip on the auto-redeem toggle to have every eligible full bank above 1 KB spend itself automatically (1 KB banks always auto-redeem, toggle or not); either way, a given size only auto-redeems once per Prestige cycle, so further full banks of that size need a manual click for the rest of the run. A bank already full when a Prestige fires stays full, giving the next run a head start. The section also shows Kilobytes' own current purchase-progress as a row of blocks, filling in regardless of whether a unit came from the main game's Buy button or from redeeming a Storage bank here.

### Changed
- The Byte Foundry no longer disappears once the main game is unlocked — a new "⚙️ Byte Foundry" link beside the page title reopens it at any time, and it stays every bit as interactive there as it is on the mandatory gate (Tap/Sacrifice/Invest never stop working, and Convert keeps working too as long as this cycle's transfer budget isn't exhausted), with its own "← Back to game" exit. It's still a mandatory gate whenever a fresh Kilobyte run is needed (before your very first Kilobytes, and after every Prestige), just no longer a one-time screen you can never see again once past it.
...
- Reaching the main game no longer requires filling Memory all the way to a full transfer budget — clicking the first transfer block (converting a 1000-bit chunk into a Kilobyte) now unlocks it immediately. Further block clicks, or the existing "Memory fills to the full budget → auto-transfer" convenience, keep working after that, sharing one running transfer budget per Prestige cycle sized to match the Kilobyte tier's own current purchase block size (8000 bits at the start of a run, growing later) — once that's spent, no block stays active (every one just shows as consumed) until your next Prestige reopens a fresh budget.
- Tapping now credits "one second's worth" at the Byte's current production rate instead of a flat 1 bit, so it scales right along with "Invest for Double Production" — previously a tap stayed stuck at 1 no matter how fast your Byte had gotten. The Byte's production now runs on an explicit 1-second base tickspeed, matching how tiers already work: "Invest for Double Production" first speeds up delivery (until the tick loop's own real-time resolution), then grows the batch size instead. It now runs on its own independent cost ladder (1/10/100/1000/10000 Bytes, …) entirely unrelated to your current capacity — so a claim never requires a full balance, only enough bits to cover that tier's own cost — and grants two claims at each of the first three tiers (1/10/100 Bytes) before dropping to one claim per tier from 1000 Bytes on.
- The Byte Foundry's tap button shrinks to roughly half its original width (while staying just as tappable, and keeping its original aspect ratio) once passive production starts, since it becomes a secondary/backup action rather than the primary loop — it deliberately shows no fill progress of its own any more, since Memory's own tile already shows the identical bits/capacity fill. The production-rate readout below 1 Byte/sec now shows an 8-block segmented bar alongside the "+N bits/sec" text; at/above 1 Byte/sec it switches to a "+N Byte(s)/sec" label instead. Every other Byte Foundry action button (Combine/Sacrifice/Invest/Build Storage Bank, and the active transfer block) shows its own fill progress toward affordability, matching the main game's Buy/Upgrade button convention, and the Invest button shows its own current cost, in Bytes. The bit balance is labeled "Memory" on its own filling tile, denominated in whatever unit actually fits (raw bits, then B/KB/MB/…/QB by 1000 each step) instead of staying in raw bits until a Byte generator exists, then Bytes forever after.
