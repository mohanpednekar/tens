...
- A new **Cache 1KB** tile on the Byte Foundry screen, next to Memory, showing progress toward the next convertible 1000-bit chunk (wraps back to 0 every time you spend one) — both tiles now fill toward their own capacity the same gradient way every button on the page already does.
- **Storage** on the Byte Foundry screen: bank a block of bits now (in KB/MB/GB/… sizes matching Kilobytes' own future per-unit level costs) to redeem later for a free Kilobyte, once Kilobytes' level actually reaches the price that block was built for. Building a bank costs 10x its own size in bits; a held bank becomes clickable once its size is at or below Kilobytes' current level cost. Banks are never lost — redeem them by hand, or flip on the new auto-redeem toggle to have every eligible bank spend itself automatically, no click needed.
...
- The Byte Foundry's tap button shrinks to roughly half its original width (while staying just as tappable, and keeping its original aspect ratio) once passive production starts, since it becomes a secondary/backup action rather than the primary loop. The production-rate readout below 1 Byte/sec now shows an 8-block segmented bar alongside the "+N bits/sec" text; at/above 1 Byte/sec it switches to a "+N Byte(s)/sec" label instead. Every Byte Foundry action button (Tap/Combine/Sacrifice/Invest/Build Storage Bank, and the active transfer block) now shows its own fill progress toward affordability, matching the main game's Buy/Upgrade button convention, and the Invest button shows its own current cost, in Bytes. The bit balance is now labeled "Memory" on its own filling tile, alongside a second always-visible tracker showing your progress (in raw bits) toward the current dynamic transfer budget — and, unlike before, it's now denominated in whatever unit actually fits (raw bits, then B/KB/MB/…/QB by 1000 each step) instead of staying in raw bits until a Byte generator exists, then Bytes forever after.
