...
- Each storage pool's own Bandwidth is now hard-capped at the square root of that pool's own Memory Capacity **converted to Bytes** (the result converted back to bits/sec), instead of always matching the raw, uncapped Byte Foundry production rate — a pool with a small Capacity window can now lag behind the raw rate once production has outgrown it. Every disk/cache fill speed and Booster live-transfer pace derives from this same capped figure, so they all re-pace automatically. Pool 1's Capacity is architecturally fixed at 1 MiB, so its Bandwidth now permanently tops out at 8,192 bits/sec (1 KiB/sec) no matter how far Speed ×2 grows the raw rate; later pools have much higher ceilings. Storage pools now display Bandwidth, Capacity, and the Memory buffer in **SI units** (matching Disks/Data Lake/caches) rather than the binary units the underlying Data Stream Capacity itself still uses internally.
