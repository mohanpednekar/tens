...
test('renders the game title and the Kilobytes tier', () => {
  ...
  expect(screen.getByRole('heading', { level: 1, name: /^data$/i })).toBeInTheDocument()
...
test('AppNav exposes accessibly-labeled Foundry, Guide, and More once unlocked', () => {
  ...
  const tiersButton = screen.getByRole('button', { name: /open data/i })
  expect(tiersButton).toHaveTextContent(/data/i)

  // Progression order: Foundry before Data in the bar.
  const nav = screen.getByRole('navigation', { name: /main navigation/i })
  const navButtons = within(nav).getAllByRole('button')
  const foundryIndex = navButtons.findIndex(b => /open byte foundry/i.test(b.getAttribute('aria-label') || ''))
  const tiersIndex = navButtons.findIndex(b => /open data/i.test(b.getAttribute('aria-label') || ''))
...
test('no redundant Game or Milestones tabs on the Factory screen', () => {
  ...
  expect(screen.getByRole('tablist', { name: /data view/i })).toBeInTheDocument()
...
test('the Guide nav item opens the Info page and Factory returns, preserving game state', async () => {
  ...
  await user.click(screen.getByRole('button', { name: /open data/i }))
  expect(screen.getByRole('heading', { level: 1, name: /^data$/i })).toBeInTheDocument()
...
test('no PP Upgrades tab or PP-based controls appear before the player has ever prestiged, even with an active autobuyer and unspent PP — Milestones stays via More', async () => {
  ...
  expect(screen.queryByRole('tab', { name: /open upgrades/i })).not.toBeInTheDocument()
  expect(screen.queryByRole('tablist', { name: /data view/i })).not.toBeInTheDocument()
...
test('a real Prestige from MainPage navigates back to the Byte Foundry, resetting Memory but keeping the generator permanent', async () => {
  ...
  expect(screen.queryByRole('heading', { level: 1, name: /^data$/i })).not.toBeInTheDocument()
  expect(screen.getByRole('heading', { level: 1, name: /byte foundry/i })).toBeInTheDocument()
...
test('a Prestige firing while voluntarily viewing the Byte Foundry turns it into the mandatory gate in place, hiding Factory but keeping Guide/More', () => {
  ...
  expect(screen.getByRole('button', { name: /tap to generate a bit/i })).toBeInTheDocument()
  expect(screen.getByRole('button', { name: /open data/i })).toBeInTheDocument()
