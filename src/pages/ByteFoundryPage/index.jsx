...
const ByteFoundryPage = ({ game, focusNonce = 0 }) => {
  ...
  const handleSacrificeClick = () => {
    ...
    const computeUnlocked = isComputeCoreConversionUnlocked(state)
    const computeNote = computeUnlocked
      ? ' This also wipes all held Compute tokens and rolls back Bandwidth upgrades bought with Compute tokens. Future Cores cost more.'
      : ' This is permanent and makes every future Core cost more.'
    if (window.confirm(
      `Sacrifice all of Memory to multiply capacity ×10, from ${formatBitsInNearestUnit(intro.capacity)} to ${formatBitsInNearestUnit(nextCapacity)}?${computeNote}`
    )) {
