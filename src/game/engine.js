...
// Whether the Upgrade Data Stream button is clickable: the Buffer is at least
// INTRO_CAPACITY_UPGRADE_ARM_FRACTION (99%) full, the byte has been combined, Capacity isn't at its
// cap, and no upgrade is already armed. Below 100% a click arms the upgrade instead of firing it
// (see pickIntroCapacityMilestone) — continuous pool-buffer / Data Lake draw could otherwise keep
// the Buffer just short of full indefinitely.
export const isMemoryCapacityUpgradeArmable = state => {
  ...
  if ((intro.bits ?? 0) < (intro.capacity ?? 0) * INTRO_CAPACITY_UPGRADE_ARM_FRACTION) return false
  return !isMemoryCapacityAtCap(state)
}

// Upgrade Data Stream click: upgrades immediately when the Buffer is full, otherwise arms the
// upgrade once the Buffer is at least 99% full. While armed, every Data Stream outflow
