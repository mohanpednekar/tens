...
// on every Prestige/Scale Up (see prestigeGame/scaleUpGame), which makes this overflow far less
...
const createEmptyDataLakeTier = () => ({
  // Whole completed lake-disk units currently banked (see getDataLakeDiskCounts/
  // decomposeDataLakeUnits below for the ×1/×10/×100 breakdown this derives) — what a Booster
  // purchase spends against.
  depositedUnits: 0,
  // Raw bits accumulated so far toward completing the NEXT lake-disk (whichever sub-size is
  // currently open — see getDataLakeCurrentFillSubSize) — reset to 0 whenever that next disk
  // completes (carrying any overshoot forward) or whenever depositedUnits is spent (a Booster
  // purchase or a capacity upgrade both discard in-progress fill on the disk that was filling).
  fillBits: 0,
  purchased: 0,
  // Whether this lake has ever completed its first (smallest, ×1) disk — a permanent latch (see
...
export const createInitialGameState = () => ({
  ...
  // Resets to 1 for every tier on Prestige and Scale Up, same as owned/purchased.
  ...
  // for every tier on Prestige and Scale Up.
  ...
  // every tier on Prestige and Scale Up, same as owned/purchased. Speeds up this tier's own
  ...
  intro: {
    ...
    // tickDiskAutoFill/pullDiskForCurrentLevel below) — "never lost," survives Prestige/Scale
    ...
    // (tickQueuedDiskBuild) the instant its own pool buffer can afford one and nothing outranks it
    // in the forced priority order, with no further click needed — and keeps re-firing, pass after
    ...
    // computeNodes themselves), but carried through untouched by Scale Up/Overclock, same as the
    ...
    // untouched by Scale Up/Overclock like the rest of intro. The Data Stream's own decaying tap
...
// getScaleUpRequirement/isLastTierTickspeedXpUnlocked/prestigeCardEverRevealed already key off.
...
export const eraGame = state => {
  ...
  const base = {
    ...
    autoScaleUp: state.autoScaleUp ?? initial.autoScaleUp,
    autoScaleUpEnabled: state.autoScaleUpEnabled ?? initial.autoScaleUpEnabled,
...
// The unconditional production-speed multiplier from Scale Up activations (see scaleUpGame):
// SCALE_UP_MULTIPLIER_BASE raised to scaleUpCount, so each activation doubles it (1x, 2x, 4x,
// 8x, …). Unlike getPrestigeProductionMultiplier, this needs no unlock purchase — it applies
// automatically as soon as scaleUpCount > 0.
export const getScaleUpMultiplier = scaleUpCount =>
  SCALE_UP_MULTIPLIER_BASE ** clampNonNegative(scaleUpCount)

// The index (see state.scaleUpTargetTierIndex) clamped into TIER_DEFINITIONS' actual bounds —
// scaleUpTargetTierIndex itself is deliberately allowed to climb past the last tier (see its own
// comment in createInitialGameState), so this is the one place that clamp is applied.
...
// The scale-up target tier's (see getScaleUpTargetTier above) LEVEL Scale Up currently requires: a
// flat TIER_UNLOCK_PREV_LEVEL_REQUIREMENT (3) while scaleUpTargetTierIndex hasn't yet reached the
// last tier — the SAME threshold that unlocks the tier one past the target (see isTierUnlocked), so
// reaching it always coincides with a new tier becoming permanently reachable too — or, once
// scaleUpTargetTierIndex has reached (and, on every further activation, climbed past) the last
// tier, a repeating multiple of SCALE_UP_FINAL_TIER_REQUIREMENT_STEP (3) that climbs by one further
// step per activation already fired since then: level 3 for the first such activation, 6 for the
// second, 9 for the third, and so on — read directly off how far scaleUpTargetTierIndex already
// sits past the last tier's own index, with no separate counter needed. Expressed as a level target
...
export const getScaleUpRequirement = state => {
  ...
  const activationsPastLastTier = clampNonNegative(state.scaleUpTargetTierIndex ?? 0) - (TIER_DEFINITIONS.length - 1)
  return SCALE_UP_FINAL_TIER_REQUIREMENT_STEP * (activationsPastLastTier + 1)
}
...
// Beyond that floor, each further claim needs one more level than the last — unlike
// getScaleUpRequirement (whose own requirement only ever grows once every tier is already
// unlocked, and by 3 per activation rather than 1), Overclock is only ever reachable once the last
// tier itself is unlocked to begin with, so it keeps its own simple +1-per-claim shape unconditionally.
// Expressed as a level target against state.purchaseLevels[lastTierId] directly (no "completed
// blocks" display offset the way Scale Up's own requirement gets — see
...
// getScaleUpRequirement/getOverclockRequirement) rather than an owned-count threshold, so it stays
...
export const tickGame = (elapsedSeconds, autobuyerBatchSize = 1) => state => {
  ...
  // Each unlocked pool's own local buffer (see tickPoolBufferFill) tops up from whatever's left of
  // the shared Data Stream AFTER tier01's own bootstrap conversion (tickIntroAutoInvest) and Queued
  // Capacity above have already had first claim this tick — pools fill from genuine leftover
  ...
  // FINAL tier levels (post autobuyer/Scale Up) — isDiskPullEligible depends on them, so a level
  ...
  const scaleUpMultiplier = getScaleUpMultiplier(stateAfterFlops.scaleUpCount ?? 0)
  ...
  TIER_DEFINITIONS.forEach(tier => {
    ...
    // Floored so owned/resources stay integer-valued: owned, ticksElapsed, scaleUpMultiplier
  ...
  // Runs last, against this tick's final tier01 level (post autobuyer/Scale Up), so a Disk
  ...
  let stateBeforeStorage = stateAfterScaleUp
...
// factors of 1000) lands on exactly 64 — there it goes to 125 instead of 128, landing back on a
// round SI figure a step earlier than a raw ×2 would.
export const getNextSiDoubledValue = bits => {
...
// big Capacity ladder above — but that fraction is 1 (the buffer's ceiling matches Capacity
// exactly), not some smaller slice. This isn't a rounding-down choice: within a single pool, the
// disk-ladder's own three step costs already span roughly a 1%/10%/90% spread of that pool's own
// Capacity ceiling (each DISK_BUILD_COST_MULTIPLIER-scaled step is DISK_LADDER_SIZE_MULTIPLIER
// bigger than the last, while a pool's own Capacity ceiling is only MEMORY_BINARY_UNIT_STEP bigger
// than the previous pool's — the two ratios are close by design, see docs/DESIGN_HISTORY.md, but
// not equal), so a pool's own LARGEST disk always costs roughly 80-95% of that pool's Capacity
// ceiling. Any meaningfully smaller fraction (the original 1/MEMORY_BINARY_UNIT_STEP this started
// as) would leave that top disk permanently unaffordable — the buffer could never hold enough,
// even fully topped up, since it caps out below the cost forever. "Capacity" and the buffer's own
// ceiling therefore match: Capacity is the pool's potential; the buffer is how much of it is
...
// — is what's left for the next pool. A pool's own reservation counts against the shared rate
// whether or not it currently has room to use all of it (the same "fixed-priority allocation,
// unused share isn't returned mid-chain" shape the forced priority order itself already uses
// elsewhere), so an earlier pool's Bandwidth cap always determines how much throughput a later
// one can ever see, never the other way around. A real bit TRANSFER out of intro.bits into that
// pool's buffer, bounded by elapsedSeconds, the pool's own remaining buffer room, and whatever
// intro.bits actually holds — never instant, matching every other Memory-derived transfer in this
// file. Called from tickGame AFTER tier01's own bootstrap conversion (tickIntroAutoInvest) and
// Queued Capacity have already had first claim on the shared Data Stream this tick — pool buffers
...
// "primary fill only" per FILL_MULTIPLIER_* in layers.js. Once a pool's own buffer is completely
// full, its reserved share feeds that pool's own matching Data Lake instead, at a fill-based rate
// (see getDataLakeOverflowRatePercent/fillDataLakeDisks in the "Data Lakes" section below) rather
// than going to waste.
export const tickPoolBufferFill = elapsedSeconds => state => {
  ...
  const poolBuffers = { ...(state.intro.poolBuffers ?? {}) }
  const dataLakes = { ...state.intro.dataLakes }
  let remainingRate = getIntroProductionRate(state.intro ?? {})
  let changed = false

  for (let poolIndex = 1; poolIndex <= unlockedCount; poolIndex += 1) {
    const poolRate = getStoragePoolBandwidth(state, poolIndex)
    const fillRate = Math.min(poolRate, remainingRate)
    remainingRate = Math.max(0, remainingRate - poolRate)
    if (fillRate <= 0 || bits <= 0) continue
...
export const normalizePoolMemoryCapacity = state => {
  ...
  if (nextIntro.dataLakes) {
    ...
    for (const tierKey of Object.keys(dataLakes)) {
      ...
      // Also clamp depositedUnits down to whatever capacity that (possibly just-clamped) level
      // actually allows — a save whose level got clamped above, or one written under an even
      // earlier deposits-shaped schema entirely, could otherwise carry more banked units than the
      // lake's own current capacity permits.
      const rawDeposited = lake.depositedUnits ?? 0
      const capacity = DATA_LAKE_CAPACITY_BY_LEVEL[level]
      const depositedUnits = Math.min(Math.max(Math.trunc(rawDeposited) || 0, 0), capacity)
      if (level !== rawLevel || depositedUnits !== rawDeposited) {
        dataLakes[tierKey] = { ...lake, capacityLevel: level, depositedUnits, fillBits: 0 }
        lakesChanged = true
      }
    }
    if (lakesChanged) {
...
export const getDiskCost = (state, capacityBits) =>
  capacityBits * getDiskProvisionPassesRequired(state, capacityBits)

// Passes already collected toward the CURRENT ladder offer's build cost (see provisionDisk) — one
// pass = the disk's own face-value size, getDiskProvisionPassesRequired(state, size) passes fund
...
// disk's own face-value size each — see getDiskProvisionPassesCollected above — rather than the
// whole cost at once, so the pool buffer only ever needs to hold one pass at a time, not the full
// lump sum. Each call collects as many WHOLE passes as the pool buffer currently affords (capped at
// however many remain) — a buffer already holding the full cost completes every pass, and starts
// the timed build, in one call, exactly as a single-payment build always has; a smaller buffer
// collects a partial installment and leaves the rest for a later call. `passesRemaining` is clamped
// at 0 (never negative) so a save carrying a stale, already-over-required `diskProvisionPasses`
...
export const provisionDisk = (state, maxPasses = Infinity) => {
  ...
  return {
    ...
    intro: {
      ...
      // Funding isn't complete yet — auto-arm the queue so the remaining passes fire themselves
      // (tickQueuedDiskBuild, unconditionally wired into tickGame's own tickStorage) as the buffer
...
// Commit to the NEXT Provision Disk build before its own pool buffer can afford even its first
// pass — the same "arm it once, it fires itself" aid as Capacity's own queueIntroCapacityUpgrade,
// and implemented/tested but likewise not exposed as its own UI control (see docs/DESIGN_HISTORY.md
...
export const tickDiskAutoFill = (elapsedSeconds = 0) => state => {
  ...
  const poolBuffers = { ...(state.intro.poolBuffers ?? {}) }
  const getPoolBufferBitsLocal = poolIndex => poolBuffers[poolIndex] ?? 0
  const poolBudgets = {}
  ...
  // Pass 1 — refill caches toward full in whole-block quanta (each pool's own buffer progress
  // stays visible), capped at CACHE_FILL_FROM_MEMORY_BANDWIDTH_MULTIPLIER times the current Byte
  // Foundry production rate — a CACHE filling FROM that pool's buffer can drain a big banked
  // balance faster than live production, but never instantly, no matter how much has piled up
  // while blocked. One shared budget per pool across every eligible size in that pool this call,
  // since it's all drawn from that pool's own Bandwidth AND its own local buffer (see
  // getPoolBufferBits/tickPoolBufferFill — the shared Data Stream Buffer no longer funds this
  // directly). Skip sizes mid-flush: their cache is locked full until the pour completes or
  // cancels.
...
// matching lake (see tickPoolBufferFill/fillDataLakeDisks below); Booster purchases at the
// matching compute tier spend that lake's own banked units.
...
// computeCoresEverEarned is a lifetime EARNED counter, deliberately separate from the live
// computeCores balance a Boost/merge can spend back down — it must only ever go up. Incrementing
// it by 1 per successful tier-1 Booster (rather than tracking `max(previous, live balance)`, an
// earlier version's bug — see docs/DESIGN_HISTORY.md) is what makes that true: earning 3, spending
...
// Feeds `overflowBits` (see the overflow branch inside tickPoolBufferFill) into lake `tierIndex`'s
// own currently-open disk slot, completing however many whole disks that amount reaches —
// smallest sub-size first — and carrying any leftover forward as fillBits progress toward the
// next one. `dataLakes` is tickPoolBufferFill's own in-progress accumulator (each pool only ever
// touches its own matching lake, so reading `dataLakes[tierIndex] ?? getDataLakeTier(state,
// tierIndex)` is always this lake's pre-this-tick state). Returns null when the lake is already
...
// docs/DESIGN_HISTORY.md) in favor of buyBooster above, which spends the matching Data Lake's own
...
// reset by Prestige/Scale Up — see prestigeGame/scaleUpGame). A no-op if this tier's autobuyer
...
// Prestige/Scale Up — see prestigeGame/scaleUpGame). A no-op if Auto-Prestige hasn't been bought
...
// buyTickspeedAutobuyer/buyAutoScaleUp.
...
// setAutoPrestigeEnabled/setAutoScaleUpEnabled/setAutoGlobalTickspeedEnabled above. A no-op if it
...
export const prestigeGame = state => {
  ...
  return applyAutobuyerMilestones({
    ...
    // scaleUpGame/overclockGame (below) are intra-cycle soft resets, not new cycles, and still
    ...
    // purchase itself does (see setAutoPrestigeEnabled/setAutoScaleUpEnabled/
    ...
    // reset tier higher: unlike an ordinary Scale Up (which scaleUpGame explicitly carries
    ...
    // owned/purchased, so a real Prestige/Scale Up still relocks every tier beyond the first
...
// re-buying the multiplier back up from scratch once Money allows. lastTierXpConsumed resets to 0
// here too, same as prestigeGame — the last tier's own owned/purchased count also resets to 0 like
// every other tier's, so the XP-funded mechanic (see isLastTierTickspeedXpUnlocked's live
// owned-vs-block-size check) is doubly disengaged: not just inactive until owned is bought back up
// to a full level, but with nothing banked to re-engage with even then, since Scale Up wipes the XP
...
// completely untouched — Scale Up is unrelated to real Prestige or Prestige Points, and doesn't
// award or spend any — but `prestige.xp` resets to 0, same as lastTierXpConsumed, since XP is a
// run-scoped currency now. `prestige.highestMilestone` (the money-exponent watermark checkMilestones
// grants further XP against) resets to the fresh initial value here too, same as prestigeGame
// already did — it must track the reset resources, not the previous run's peak, or a fresh run would
// earn no XP at all until money climbs back past wherever the last run left off (previously an
// asymmetry between the two reset paths — see docs/DESIGN_HISTORY.md). A no-op (returns the same
// state) while frozen (a frozen state is waiting on a real Prestige, not a Scale Up) or before the
// scale-up target tier has reached that cycle's requirement.
export const scaleUpGame = state => {
  ...
  return {
    ...
    scaleUpCount: (state.scaleUpCount ?? 0) + 1,
    // Unconditional +1 — see this function's own comment above and scaleUpTargetTierIndex's own
    // comment in createInitialGameState for why no separate phase check is needed here.
    scaleUpTargetTierIndex: clampNonNegative(state.scaleUpTargetTierIndex ?? 0) + 1,
...
// that cycle's requirement — same guards as scaleUpGame.
export const overclockGame = state => {
  ...
  return {
    ...
    intro: state.intro ?? initial.intro,
    autobuyers: state.autobuyers ?? initial.autobuyers,
    // Same permanence as scaleUpGame/prestigeGame give these two "enabled" flags — see there.
    autobuyersEnabled: state.autobuyersEnabled ?? initial.autobuyersEnabled,
    smartAutobuyer: state.smartAutobuyer ?? initial.smartAutobuyer,
    tierTickspeedAutobuyer: state.tierTickspeedAutobuyer ?? initial.tierTickspeedAutobuyer,
    tierTickspeedAutobuyerEnabled: state.tierTickspeedAutobuyerEnabled ?? initial.tierTickspeedAutobuyerEnabled,
    autoPrestige: state.autoPrestige ?? initial.autoPrestige,
    // Same permanence as scaleUpGame/prestigeGame give these four "enabled" flags above — see there.
    autoPrestigeEnabled: state.autoPrestigeEnabled ?? initial.autoPrestigeEnabled,
    autoPrestigeAutobuyer: state.autoPrestigeAutobuyer ?? initial.autoPrestigeAutobuyer,
    autoPrestigeAutobuyerEnabled: state.autoPrestigeAutobuyerEnabled ?? initial.autoPrestigeAutobuyerEnabled,
    prestigeSpeedBonusUnlocked: state.prestigeSpeedBonusUnlocked ?? initial.prestigeSpeedBonusUnlocked,
    ...
    autoGlobalTickspeed: state.autoGlobalTickspeed ?? initial.autoGlobalTickspeed,
    autoGlobalTickspeedEnabled: state.autoGlobalTickspeedEnabled ?? initial.autoGlobalTickspeedEnabled,
    // lastTierXpConsumed/everUnlockedTierIds are NOT carried over — same reasoning as scaleUpGame,
    // see there.
...
export const buyAutoScaleUp = state => {
  if (isProductionFrozen(state)) return state
  if (state.autoScaleUp) return state
  if (clampNonNegative(state.prestige.points) < AUTO_SCALE_UP_COST) return state

  const latched = latchComputeFlopsPageUnlocked(state)
  return {
    ...latched,
    prestige: { ...latched.prestige, points: latched.prestige.points - AUTO_SCALE_UP_COST },
    autoScaleUp: true,
  }
}
...
// aren't enough unspent points, or while production is frozen — same convention as buyAutoScaleUp.
...
// Toggles whether Auto Scale Up currently acts (see autoScaleUpEnabled/tickGame) — same
// unconditional, permanent-preference convention as setAutoPrestigeEnabled above. A no-op if
// Auto Scale Up hasn't been bought yet.
export const setAutoScaleUpEnabled = enabled => state => {
  if (!state.autoScaleUp) return state
  return { ...state, autoScaleUpEnabled: !!enabled }
}
...
// points, or while production is frozen — same convention as buyAutoScaleUp/buyPrestigeSpeedBonus.
...
// setAutoPrestigeEnabled/setAutoScaleUpEnabled above. A no-op if it hasn't been bought yet.
