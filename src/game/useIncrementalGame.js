...
import { activateComputeBoost, applyOfflineProgress, buyAutoPrestige, buyAutoPrestigeAutobuyer, buyAutoScaleUp, buyComputeAutoBoost, buyComputeFlopsTier, buyGlobalTickspeedMultiplier, buyHyperscaler, buyPrestigeDoublePp, buyPrestigeSpeedBonus, buySmartAutobuyer, buyTickspeedAutobuyer, buyTickspeedMultiplier, buyTierQuantity, clearDiskBuildQueue, clearIntroCapacityUpgradeQueue, combineIntroByte, consumeXpForLastTierTickspeed, convertIntroBitsToKilobytes, createInitialGameState, doubleDataLakeCapacity, enableAutoMergeClustersIntoNetwork, enableAutoMergeCloudsIntoDatacenter, enableAutoMergeCoresIntoNode, enableAutoMergeDatacentersIntoSupercomputer, enableAutoMergeFabricsIntoCloud, enableAutoMergeGridsIntoFabric, enableAutoMergeNetworksIntoGrid, enableAutoMergeNodesIntoCluster, enableAutoMergeSupercomputersIntoMegacomputer, eraGame, getOfflineEffectiveSeconds, latchComputeFlopsPageUnlocked, mergeComputeClustersIntoNetwork, mergeComputeCloudsIntoDatacenter, mergeComputeCoresIntoNode, mergeComputeDatacentersIntoSupercomputer, mergeComputeFabricsIntoCloud, mergeComputeGridsIntoFabric, mergeComputeNetworksIntoGrid, mergeComputeNodesIntoCluster, mergeComputeSupercomputersIntoMegacomputer, normalizePoolMemoryCapacity, overclockGame, pickIntroCapacityMilestone, pinMuseumEntry, prestigeGame, provisionDisk, queueDiskBuild, reclaimComputeBoost, forfeitComputeBoost, resetByteFoundry, resetStoragePool, setAutobuyerEnabled, setAutoGlobalTickspeedEnabled, setAutoPrestigeAutobuyerEnabled, setAutoPrestigeEnabled, setAutoScaleUpEnabled, setComputeAutoBoostType, setComputeFlopsAutobuyerEnabled, setTierTickspeedAutobuyerEnabled, scaleUpGame, stackComputeBoost, startComputeCloudsMerge, startComputeClustersMerge, startComputeCoresMerge, startComputeDatacentersMerge, startComputeFabricsMerge, startComputeGridsMerge, startComputeNetworksMerge, startComputeNodesMerge, startComputeSupercomputersMerge, startDataLakeAutoConvert, tapIntroBit, tapPoolBuffer, tickGame, unpinMuseumEntry, upgradeComputeMergeDuration } from './engine'
...
export const useIncrementalGame = () => {
  ...
  const actions = useMemo(() => ({
    ...
    scaleUp: () => setState(scaleUpGame),
    overclock: () => setState(overclockGame),
    buyAutoScaleUp: () => setState(buyAutoScaleUp),
    ...
    setAutoScaleUpEnabled: enabled => setState(setAutoScaleUpEnabled(enabled)),
    ...
    tapPoolBuffer: poolIndex => setState(tapPoolBuffer(poolIndex)),
    combineIntroByte: () => setState(combineIntroByte),
    pickIntroCapacityMilestone: () => setState(pickIntroCapacityMilestone),
    clearIntroCapacityUpgradeQueue: () => setState(clearIntroCapacityUpgradeQueue),
    convertIntroBitsToKilobytes: () => setState(convertIntroBitsToKilobytes),
    provisionDisk: () => setState(provisionDisk),
    queueDiskBuild: () => setState(queueDiskBuild),
    clearDiskBuildQueue: () => setState(clearDiskBuildQueue),
    doubleDataLakeCapacity: tierIndex => setState(doubleDataLakeCapacity(tierIndex)),
    ...
    startDataLakeAutoConvert: tierIndex => setState(startDataLakeAutoConvert(tierIndex)),
    resetStoragePool: tierIndex => setState(resetStoragePool(tierIndex)),
  ...
  // can never mutate a real player's live save. Runs the result through
  // normalizePoolMemoryCapacity — the same defensive clamp a real save load applies — since this
  // is the one write path that bypasses loadGameState's own mergeState/normalization pipeline
  // entirely; without it, a manual edit to a bounded field like intro.dataLakes[n].capacityLevel
  // (e.g. via the Variables tree) could set it past DATA_LAKE_CAPACITY_BY_LEVEL's own bounds and
  // silently break every downstream check against it for the rest of the Dev Mode session.
  const setDevState = useCallback(updater => {
    if (!isDevModeActive()) return
    setState(prev => normalizePoolMemoryCapacity(updater(prev)))
  }, [])
