...
const computeInitialGame = () => {
  ...
  const result = computeOfflineCatchUp(elapsedRealSeconds, loaded) ?? { state: loaded, offlineProgress: null }
  return { ...result, state: latchComputeFlopsPageUnlocked(result.state) }
}
