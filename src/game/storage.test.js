...
describe('Dev Mode', () => {
  ...
  it('applyDevGameStateJson deep-merges nested containers, preserving untouched siblings two levels down', () => {
    ...
    const currentState = {
      ...
      intro: {
        ...
        dataLakes: {
          ...
          1: { deposits: { 1: 50, 10: 3, 100: 0 }, purchased: 5, transfers: [], slotMax: 9 },
          2: { deposits: { 1: 0, 10: 9, 100: 1 }, purchased: 12, transfers: [], slotMax: 9 },
        },
      },
    }
    const result = applyDevGameStateJson(
    ...
    expect(result.state.intro.dataLakes['2']).toEqual({ deposits: { 1: 0, 10: 9, 100: 1 }, purchased: 12, transfers: [], slotMax: 9 })
  })
